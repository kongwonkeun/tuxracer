...
/* Add a new node name to the node name hash table. node_name contains the child's name only. */
static int add_scene_node( char *parent_name, char *node_name, scene_node_t *node ) 
{
    Tcl_HashEntry *entry;
    int newEntry;
    char new_name[1024];
...
int get_scene_node( char *node_name, scene_node_t **node ) 
{
    Tcl_HashEntry *entry;
...
int add_material( char *mat_name, material_t *mat ) 
{
    Tcl_HashEntry *entry;
    int newEntry;
...
int get_material( char *mat_name, material_t **mat ) 
{
    Tcl_HashEntry *entry;
...
char* create_scene_node( char *parent_name, char *child_name, scene_node_t **node )
{
...
char* rotate_scene_node( char *node, char axis, scalar_t angle ) 
{
    scene_node_t *nodePtr;
...
char* translate_scene_node( char *node, vector_t vec ) 
{
    scene_node_t *nodePtr;
...
char* scale_scene_node( char *node, point_t center, scalar_t factor[3] ) 
{
    scene_node_t *nodePtr;
    matrixgl_t matrix;
...
char* transform_scene_node( char *node, matrixgl_t mat, matrixgl_t invmat ) 
{
    scene_node_t *nodePtr;
...
char* set_scene_node_material( char *node, char *mat ) 
{
    material_t *matPtr;
    scene_node_t *nodePtr;
...
char* set_scene_node_shadow_state( char *node, char *state ) 
{
    scene_node_t *nodePtr;
...
char* set_scene_node_eye( char *node, char *which_eye )
{
    scene_node_t *nodePtr;
...
char* create_tranform_node( char *parent_name, char *child_name ) 
{
    scene_node_t *node;
    char *msg;
...
char* create_sphere_node( char *parent_name, char *child_name, scalar_t resolution ) 
{
    scene_node_t *node;
    char *msg;
    ...
    node->param.sphere.divisions = min( MAX_SPHERE_DIVISIONS, max( MIN_SPHERE_DIVISIONS, ROUND_TO_NEAREST( getparam_tux_sphere_divisions() * resolution ) ) );

    return NULL;
}

char* create_material( char *mat, colour_t diffuse, colour_t specular_colour, scalar_t specular_exp ) 
{
    material_t *matPtr;
