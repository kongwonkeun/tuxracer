...
void init_joystick()
{
    ...
    print_debug(DEBUG_JOYSTICK, "joystick has %d button%s", num_buttons, num_buttons == 1 ? "" : "s");

    /* get number of axes */
    num_axes = SDL_JoystickNumAxes(joystick);
    print_debug(DEBUG_JOYSTICK, "joystick has %d ax%ss", num_axes, num_axes == 1 ? "i" : "e");
}
...
scalar_t get_joystick_x_axis()
{
    ...
    check_assertion( joystick != NULL, "joystick is null" );
    ...
    if ( axis >= num_axes || axis < 0 ) {
        ...
        if ( !warning_given ) {
            print_warning( IMPORTANT_WARNING, "joystick x axis mapped to axis %d but joystick only has %d axes", axis, num_axes );
            warning_given = True;
        }
        return 0.0;
    }
...
scalar_t get_joystick_y_axis()
{
    ...
    if ( axis >= num_axes || axis < 0 ) {
        ...
        if ( !warning_given ) {
            print_warning( IMPORTANT_WARNING, "joystick y axis mapped to axis %d but joystick only has %d axes", axis, num_axes );
            warning_given = True;
        }
        return 0.0;
    }
...
bool_t is_joystick_button_down( int button ) 
{
    ...
    if ( button >= num_buttons ) {
        if ( !warning_given ) {
            print_warning( IMPORTANT_WARNING, "state of button %d requested, but joystick only has %d buttons.  Further warnings of this type will be suppressed", button, num_buttons );
            warning_given = True;
        }
        return False;
    }
...
#endif /* defined(HAVE_SDL) && defined(HAVE_SDL_JOYSTICKOPEN) */
