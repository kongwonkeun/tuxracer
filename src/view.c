...
/* Time constant for interpolation of camera orientation in "behind" mode (s) */
...
/* Time constant for interpolation of orientation position in "follow" mode (s) */
#define FOLLOW_ORIENT_TIME_CONSTANT 0.06

/* This places an upper limit on the interpolation, which prevent excessive "jumping" of the camera during the occaisonal slow frame */
...
void traverse_dag_for_view_point(scene_node_t *node, matrixgl_t trans)
{
    ...
    if (node->eye == True) {
        set_tux_eye(node->which_eye, transform_point(new_trans, make_point( 0., 0., 0.)));
    }
    child = node->child;
    while (child != NULL) {
...
{ 
    ...
    tux_view_pt = move_point(tux_view_pt, scale_vector(0.2, plyr->plane_nml));
    return tux_view_pt; 
}
...
point_t interpolate_view_pos(point_t plyr_pos1, point_t plyr_pos2, scalar_t max_vec_angle, point_t pos1, point_t pos2, scalar_t dist, scalar_t dt, scalar_t time_constant)
{
    ...
    if (theta > max_vec_angle)
    {
        ...
        make_rotation_about_vector_matrix(rot_mat, axis, theta-max_vec_angle);
        vec2 = transform_vector(rot_mat, vec2);
    }
    return move_point(plyr_pos2, scale_vector(dist, vec2));
}

void interpolate_view_frame(vector_t up1, vector_t dir1, vector_t *p_up2, vector_t *p_dir2, scalar_t dt, scalar_t time_constant)
{
    quaternion_t q1, q2;
    scalar_t alpha;
    vector_t x1, y1, z1;
    ...
    z1 = scale_vector(-1.0, dir1); normalize_vector(&z1);
    y1 = project_into_plane(z1, up1); normalize_vector(&y1);
    x1 = cross_product(y1, z1); make_change_of_basis_matrix(cob_mat1, inv_cob_mat1, x1, y1, z1);
    q1 = make_quaternion_from_matrix(cob_mat1);

    z2 = scale_vector(-1.0, *p_dir2); normalize_vector(&z2);
    y2 = project_into_plane(z2, *p_up2); normalize_vector(&y2);
    x2 = cross_product(y2, z2); make_change_of_basis_matrix(cob_mat2, inv_cob_mat2, x2, y2, z2);
...
void update_view(player_data_t* plyr, scalar_t dt)
{
    ...
    time_constant_mult = 1.0 / min(1.0, max(0.0, (speed - NO_INTERPOLATION_SPEED) / (BASELINE_INTERPOLATION_SPEED - NO_INTERPOLATION_SPEED)));
    ...
    case BEHIND:
    {
        ...
        view_vec = make_vector( 
            0, 
            sin(ANGLES_TO_RADIANS(course_angle - CAMERA_ANGLE_ABOVE_SLOPE + PLAYER_ANGLE_IN_CAMERA)),
            cos(ANGLES_TO_RADIANS(course_angle - CAMERA_ANGLE_ABOVE_SLOPE + PLAYER_ANGLE_IN_CAMERA)) 
        );
        ...
        if (plyr->view.initialized) {
            ...
            int i;
            for (i = 0; i < 2; i++) {
        	view_pt = interpolate_view_pos(plyr->pos, plyr->pos, MAX_CAMERA_PITCH, plyr->view.pos, view_pt, CAMERA_DISTANCE, dt, BEHIND_ORBIT_TIME_CONSTANT * time_constant_mult);
            }
        }
    ...
    view_vec = subtract_points(view_pt, plyr->pos);
        ...
        make_rotation_about_vector_matrix(rot_mat, axis, PLAYER_ANGLE_IN_CAMERA);
        ...
        if (plyr->view.initialized) {
            ...
            int i;
            for (i = 0; i < 2; i++) {
                interpolate_view_frame(plyr->view.up, plyr->view.dir, &up_dir, &view_dir, dt, BEHIND_ORIENT_TIME_CONSTANT);
    ...
    case FOLLOW: 
    {
        ...
        if (plyr->view.initialized) {
            ...
            int i;
            for (i = 0; i < 2; i++) {
                view_pt = interpolate_view_pos(plyr->view.plyr_pos, plyr->pos, MAX_CAMERA_PITCH, plyr->view.pos, view_pt, CAMERA_DISTANCE, dt, FOLLOW_ORBIT_TIME_CONSTANT * time_constant_mult);
            }
        }
        ...
        if (plyr->view.initialized) {
            ...
            int i;
            for (i = 0; i < 2; i++) {
                interpolate_view_frame(plyr->view.up, plyr->view.dir, &up_dir, &view_dir, dt, FOLLOW_ORIENT_TIME_CONSTANT);
                up_dir = make_vector(0.0, 1.0, 0.0);
            }
        }
        break;
