...
void draw_game_over_text(void)
{
    ...
    if (!get_font_binding("race_over", &font)) {
        print_warning(IMPORTANT_WARNING, "couldn't get font for binding race_over");
    } else {
        string = "Race Over";
        get_font_metrics(font, string, &string_w, &asc, &desc);
        glPushMatrix();
        {
            glTranslatef(x_org + box_width / 2.0 - string_w / 2.0, y_org + box_height - asc, 0);
    ...
    if (!g_game.race_aborted) {
        if (!get_font_binding("race_stats_label", &stat_label_font) || !get_font_binding("race_stats", &font)) {
            print_warning(IMPORTANT_WARNING, "Couldn't get fonts for race stats");
        } else {
            int asc2;
            int desc2;
            ...
            glPushMatrix();
            {
                ...
                int seconds;
                int hundredths;
                glTranslatef(x_org, y_org + 150, 0);
            ...
            glPushMatrix();
            {
                glTranslatef(x_org, y_org + 150 - (asc + desc), 0);
                bind_font_texture(stat_label_font);
            ...
            glPushMatrix();
            {
                glTranslatef(x_org, y_org + 150 - 2 * (asc + desc), 0);
                bind_font_texture(stat_label_font);
    ...
    if (g_game.race_aborted) {
        string = "Race aborted.";
    } else if ((g_game.practicing || is_current_cup_complete()) && did_player_beat_best_results()) {
    ...
    if (!get_font_binding("race_result_msg", &font)) {
        print_warning(IMPORTANT_WARNING, "couldn't get font for binding race_result_msg");
    } else {
        get_font_metrics(font, string, &string_w, &asc, &desc);
        glPushMatrix();
        {
            glTranslatef(x_org + box_width / 2. - string_w / 2., y_org + desc, 0);
            bind_font_texture(font);
            draw_string(font, string);
        }
        glPopMatrix();
...
{
    int status = 0;
    status |= add_keymap_entry(GAME_OVER, DEFAULT_CALLBACK, NULL, NULL, game_over_cb);
