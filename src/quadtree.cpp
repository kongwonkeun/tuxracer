...
quadsquare::quadsquare(quadcornerdata* pcd) // Constructor.
{
    pcd->Square = this;
    ...
    if ( pcd->Parent == NULL ) {
        print_debug( DEBUG_QUADTREE, (char *)"initializing root node" );
...
quadsquare::~quadsquare() // Destructor.
{
...
// Also updates MinY & MaxY.
{
    ...
    for (i=cd.xorg; i<=cd.xorg+whole; i++) {
        for (j=cd.zorg; j<=cd.zorg+whole; j++) {
            ...
            if ( i < 0 || i >= RowSize || j < 0 || j >= NumRows ) 
            {
                continue;
            }

            terrain = (int) Terrain[ i + RowSize*j ];
            check_assertion( terrain >= 0 && terrain < NumTerrains, "Invalid terrain type" );
...
// Does the actual work of updating enabled states and tree growing/shrinking.
{
    ...
    // East vert.
    if ( (EnabledFlags & 1) == 0 &&  VertexTest(cd.xorg + whole, Vertex[1].Y, cd.zorg + half, Error[0], ViewerLocation, cd.Level, East) == true ) 
    {
        EnableEdgeVertex(0, false, cd);	
    }

    // South vert.
    if ( (EnabledFlags & 8) == 0 && VertexTest(cd.xorg + half, Vertex[4].Y, cd.zorg + whole, Error[1], ViewerLocation, cd.Level, South) == true ) 
    {
    ...
    if ( (EnabledFlags & 1) && SubEnabledCount[0] == 0 && VertexTest(cd.xorg + whole, Vertex[1].Y, cd.zorg + half, Error[0], ViewerLocation, cd.Level, East) == false ) 
    {
        EnabledFlags &= ~1;
        quadsquare*	s = GetNeighbor(0, cd);
    ...
    if ( (EnabledFlags & 8) && SubEnabledCount[1] == 0 && VertexTest(cd.xorg + half, Vertex[4].Y, cd.zorg + whole, Error[1], ViewerLocation, cd.Level, South) == false ) 
    {
        EnabledFlags &= ~8;
        quadsquare*	s = GetNeighbor(3, cd);
    ...
    if ( EnabledFlags == 0 && cd.Parent != NULL && BoxTest(cd.xorg, cd.zorg, whole, MinY, MaxY, CenterError, ViewerLocation) == false )
    {
        // Disable ourself.
...
void quadsquare::DrawTris()
{
    ...
    if (glLockArraysEXT_p && getparam_use_cva()) {
        if (getparam_cva_hack()) {
            /* This is a hack that seems to fix the "psychedelic colours" on some drivers (TNT/TNT2, for example) */
            if (tmp_min_idx == 0) {
                tmp_min_idx = 1;
            }
        } 
...
// Returns the number of triangles rendered.
{
    ...
    /*
     * Draw the "special" triangles that have different terrain types at each of the corners 
     */
    if (getparam_terrain_blending() && getparam_perfect_terrain_blending()) {
        ...
        if (VertexArrayCounter != 0) {
            ...
            for (j = 0; j < NumTerrains; j++) {
                ...
                /* Set alpha values */
                for (i = 0; i < VertexArrayCounter; i++) {
                    colorval(VertexArrayIndices[i], 3) = (Terrain[VertexArrayIndices[i]] == (terrain_t)j) ? 255 : 0;
                }
                DrawTris();
            }
            /* Render Ice with environment map */
            if (getparam_terrain_envmap()) {
                ...
                for (i = 0; i < VertexArrayCounter; i++) {
                    colorval(VertexArrayIndices[i], 3) = (Terrain[VertexArrayIndices[i]] == Ice) ? ENV_MAP_ALPHA : 0;
                }
                DrawEnvmapTris();
            }
        }
...
{
    if ((VertexTerrains[a] == terrain || VertexTerrains[b] == terrain || VertexTerrains[c] == terrain))
    { 
...
inline void quadsquare::MakeSpecialTri(int a, int b, int c, int terrain) 
{
    ...
    if (VertexTerrains[a] != VertexTerrains[b] && VertexTerrains[a] != VertexTerrains[c] && VertexTerrains[b] != VertexTerrains[c]) 
    { 
        VertexArrayIndices[VertexArrayCounter++] = VertexIndices[a]; 
        update_min_max(VertexIndices[a]);
        VertexArrayIndices[VertexArrayCounter++] = VertexIndices[b]; 
...
{
    if ( ( VertexTerrains[a] == terrain || VertexTerrains[b] == terrain || VertexTerrains[c] == terrain ) && ( VertexTerrains[a] >= terrain && VertexTerrains[b] >= terrain && VertexTerrains[c] >= terrain ) )
    { 
        VertexArrayIndices[VertexArrayCounter++] = VertexIndices[a]; 
        setalphaval(a); 
        update_min_max(VertexIndices[a]);
