...
static int fog_cb (ClientData cd, Tcl_Interp *ip, int argc, char *argv[]) 
{
    scalar_t tmp_arr[4];
    double tmp_dbl;
    bool_t error = False;
    ...
    while ( !error && argc > 0 ) {
        ...
        if ( strcmp( "-on", *argv ) == 0 ) {
            ...
            if ( strcmp( "exp", *argv ) == 0 ) {
                ...
                print_warning( TCL_WARNING, "tux_fog: mode must be one of `exp', `exp2', or `linear'" );
                error = True;
            }
             ...
             strcmp( "-color",  *argv ) == 0 ) 
        {
            ...
            print_warning( TCL_WARNING, "tux_fog: unrecognized parameter `%s'", *argv );
        }
    ...
    if ( error ) {
        ...
        Tcl_AppendResult( ip, "\nUsage: tux_fog [-on|-off] [-mode [exp|exp2|linear]] [-density <value>] [-start <value>] [-end <value>] [-colour { r g b a }] ", (char *) 0 );
        return TCL_ERROR;
    }
