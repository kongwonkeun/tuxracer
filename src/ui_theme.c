...
void ui_draw_menu_decorations()
{
    ...
    /* bottom left */
    if (!get_texture_binding(bl, &texobj)) texobj = 0;
    glBindTexture(GL_TEXTURE_2D, texobj);
    draw_quad(0, 0, 256, 256);

    /* bottom right */
    if (!get_texture_binding(br, &texobj)) texobj = 0;
    glBindTexture(GL_TEXTURE_2D, texobj);
    draw_quad(w - 256, 0, 256, 256);

    /* top left */
    if (!get_texture_binding(tl, &texobj)) texobj = 0;
    glBindTexture(GL_TEXTURE_2D, texobj);
    draw_quad(0, h - 256, 256, 256);

    /* top right */
    if (!get_texture_binding(tr, &texobj)) texobj = 0;
    glBindTexture(GL_TEXTURE_2D, texobj);
    draw_quad(w - 256, h - 256, 256, 256);

    /* title */
    if (!get_texture_binding(title, &texobj)) texobj = 0;
    glBindTexture(GL_TEXTURE_2D, texobj);
