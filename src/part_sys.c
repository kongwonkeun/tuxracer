...
void create_new_particles(point_t loc, vector_t vel, int num) 
{
    ...
    for (i = 0; i < num; i++) {
        ...
        newp->vel = add_vectors( 
            ...
            make_vector(VARIANCE_FACTOR * (frand() - 0.5) * speed, VARIANCE_FACTOR * (frand() - 0.5) * speed, VARIANCE_FACTOR * (frand() - 0.5) * speed) 
        );
    }
} 
...
void update_particles(scalar_t time_step)
{
    ...
    for (p = &head; *p != NULL; ) {
        ...
        (**p).pt = move_point((**p).pt, scale_vector(time_step, (**p).vel));
...
void draw_particles(player_data_t *plyr)
{
    ...
    for (p = head; p != NULL; p = p->next) {
        ...
        glColor4f(particle_colour[0], particle_colour[1], particle_colour[2], particle_colour[3] * p->alpha);
        draw_billboard(plyr, p->pt, p->cur_size, p->cur_size, False, min_tex_coord, max_tex_coord);
    } 
}
...
static int particle_colour_cb(ClientData cd, Tcl_Interp *ip, int argc, char *argv[]) 
{
    scalar_t tmp_arr[4];
    bool_t error = False;
    ...
    if (argc == 2) {
        ...
        if (argc < 3) {
            ...
            NEXT_ARG;
            print_warning(DEPRECATION_WARNING, "This format for tux_particle_colour is deprecated. The new format is:\n\ttux_particle_colour {r g b a}");

            while (!error && argc > 0) {
                if (strcmp("-ambient_and_diffuse", *argv) == 0 || strcmp("-diffuse",  *argv) == 0) 
                {
                    NEXT_ARG;
                    if (argc == 0) {
                        error = True;
                    ...
                    print_warning(TCL_WARNING, "tux_particle_colour: unrecognized parameter `%s'", *argv);
                }
                NEXT_ARG;
            }
        }
