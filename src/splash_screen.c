...
static char* logo_bindings[4] = { "splash_screen_tl", "splash_screen_bl", "splash_screen_tr", "splash_screen_br" };
...
static void draw_logo()
{
    ...
    for (i = 0; i < 4; i++) {
        ...
        glBegin(GL_QUADS);
        glTexCoord2f(0.0, 0.0); glVertex2f(ll.x, ll.y);
        glTexCoord2f(1.0, 0.0); glVertex2f(ur.x, ll.y);
        glTexCoord2f(1.0, 1.0); glVertex2f(ur.x, ur.y);
        glTexCoord2f(0.0, 1.0); glVertex2f(ll.x, ur.y);
        glEnd();
    }
}
...
{
    int status = 0;
    status |= add_keymap_entry(SPLASH, DEFAULT_CALLBACK, NULL, NULL, splash_screen_cb);
    status |= add_keymap_entry(ALL_MODES, FIXED_KEY, "tab", NULL, toggle_snow);
    check_assertion(status == 0, "out of keymap entries");
    register_loop_funcs(SPLASH, splash_screen_init, splash_screen_loop, NULL);
}
