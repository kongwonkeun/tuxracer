...
class vector // 3-element vector class, for 3D math.
{
public:
    vector() { x[0] = 0; x[1] = 0; x[2] = 0; }
...
inline float vector::operator*(const vector& v) const // dot product.
{
    float   result;
    result = x[0] * v.x[0];
    result += x[1] * v.x[1];
...
inline vector&	vector::operator+=(const vector& v) // adds a vector to *this.
{
    x[0] += v.x[0];
    x[1] += v.x[1];
    x[2] += v.x[2];
...
inline vector&	vector::operator-=(const vector& v) // subtracts a vector from *this.
{
    x[0] -= v.x[0];
    x[1] -= v.x[1];
    x[2] -= v.x[2];
...
class matrix // 3x4 matrix class, for 3D transformations.
{
public:
    matrix() { Identity(); }
