...
typedef struct {
    ...
    scalar_t kern_width; /* distance that this char takes when rendered  as part of a string */
}
tfm_char_data_t;
...
if (fread((addr), (bytes), 1, (file)) != 1) { \
    ...
    if ((swap)) { \
        if ((bytes) == 4) { \
            ...
            check_assertion(0, "can't byte-swap storage with size != 2 of 4 bytes"); \
        } \
    } \
}
...
tex_font_metrics_t* load_tex_font_metrics(char *filename)
{
    ...
    check_assertion(sizeof(short) == 2, "this architecture's short integer size is != 2");
    check_assertion(sizeof(char)  == 1, "this architecture's char size is != 1");

    /* open file */
    tfm_file = fopen(filename, "rb");
    if (tfm_file == NULL) {
        print_warning(MISSING_FILE_WARNING, "couldn't open font metrics file %s", filename);
        return NULL;
    }
    ...
    if (strncmp(magic, "\377tfm", 4) != 0) { // 0xff,t,f,m
    ...
    for (i = 0; i < num_chars; i++) {
        ...
        READ_BYTES(tfm_file, &ch_dims.ch,         sizeof(short), swap_bytes);
        READ_BYTES(tfm_file, &ch_dims.w,          sizeof(char),  False);
        READ_BYTES(tfm_file, &ch_dims.h,          sizeof(char),  False);
        ...
        cd->tex_ll = make_point2d(ch_dims.x_pixel / (scalar_t)texture_width + sstep,          ch_dims.y_pixel / (scalar_t)texture_height + tstep);
        cd->tex_lr = make_point2d(cd->tex_ll.x + sstep + ch_dims.w / (scalar_t)texture_width, cd->tex_ll.y + tstep);
        cd->tex_ur = make_point2d(cd->tex_lr.x + sstep,                                       cd->tex_lr.y + tstep + ch_dims.h / (scalar_t)texture_height);
        cd->tex_ul = make_point2d(cd->tex_ur.x + sstep - ch_dims.w / (scalar_t)texture_width, cd->tex_ur.y + tstep);
...
{
    int i;
    if (tfm->char_data[(int)c] != NULL) return tfm->char_data[(int)c];
    else if (isupper(c) && tfm->char_data[tolower(c)] != NULL) return tfm->char_data[tolower(c)];
    else if (islower(c) && tfm->char_data[toupper(c)] != NULL) return tfm->char_data[toupper(c)];
    else if (tfm->char_data[' '] != NULL) {
        print_warning(IMPORTANT_WARNING, "font does not have a representation of character `%c'; using space as placeholder", c);
        return tfm->char_data[' '];
    } else {
        for (i = 0; i < MAX_TEX_FONT_CHARS; i++) {
            if (tfm->char_data[i] != NULL) {
                print_warning(IMPORTANT_WARNING, "font does not have a representation of character `%c'; using `%c' as placeholder", c, i);
...
void get_tex_font_string_bbox(tex_font_metrics_t* tfm, char* string, int* width, int* max_ascent, int* max_descent)
{
    int i;
    int len;
    tfm_char_data_t *cd;
...
void draw_tex_font_char(tex_font_metrics_t* tfm, char c)
{
    ...
    glBegin(GL_QUADS);
    glTexCoord2dv((scalar_t*)&cd->tex_ll); glVertex2dv((scalar_t*)&cd->ll);
    glTexCoord2dv((scalar_t*)&cd->tex_lr); glVertex2dv((scalar_t*)&cd->lr);
    glTexCoord2dv((scalar_t*)&cd->tex_ur); glVertex2dv((scalar_t*)&cd->ur);
    glTexCoord2dv((scalar_t*)&cd->tex_ul); glVertex2dv((scalar_t*)&cd->ul);
    glEnd();
