...
{
    char *val;
    check_assertion(p->type == PARAM_STRING, "configuration parameter type mismatch");
...
{
    char *str_val;
    check_assertion(p->type == PARAM_CHAR, "configuration parameter type mismatch");
...
void fetch_param_int(struct param *p)
{
    char *str_val;
    int val;
    check_assertion(p->type == PARAM_INT, "configuration parameter type mismatch");
    
    str_val = Tcl_GetVar(g_game.tcl_interp, p->name, TCL_GLOBAL_ONLY);
    if (str_val == NULL || Tcl_GetInt(g_game.tcl_interp, str_val, &val) == TCL_ERROR) {
...
void fetch_param_bool(struct param *p)
{
    ...
    int val;
    bool_t no_val = False;
    check_assertion(p->type == PARAM_BOOL, "configuration parameter type mismatch");
...
#define FN_PARAM_BOOL(name)   FN_PARAM(name, bool, bool_t)
...
struct params {
    ...
    struct param audio_freq_mode; /* 0 = 11025, 1 = 22050, 2 = 44100 */
    struct param audio_format_mode; /* 0 = 8 bits, 1 = 16 bits */
    ...
    struct param mipmap_type; /* 0 = GL_NEAREST, 1 = GL_LINEAR, 2 = GL_NEAREST_MIPMAP_NEAREST, 3 = GL_LINEAR_MIPMAP_NEAREST, 4 = GL_NEAREST_MIPMAP_LINEAR, 5 = GL_LINEAR_MIPMAP_LINEAR */
    struct param ode_solver; /* 0 = Euler, 1 = ODE23, 2 = ODE45 */
    struct param fov; 
    struct param debug; 
...
{
    INIT_PARAM_STRING( data_dir, DATA_DIR, "# The location of the Tux Racer data files" );
    ...
    INIT_PARAM_INT( tux_shadow_sphere_divisions, 3, "# [EXPERT] The level of subdivision of Tux's shadow." );
    ...
    INIT_PARAM_BOOL( display_fps, False, "# Set this to true to display the current framerate in Hz." );
    INIT_PARAM_INT( x_resolution, 640, "# The horizontal size of the Tux Racer window" );
    INIT_PARAM_INT( y_resolution, 480, "# The vertical size of the Tux Racer window" );
    INIT_PARAM_BOOL( capture_mouse, False,
        "# If true, then the mouse will not be able to leave the \n"
        "# Tux Racer window.\n"
        "# If you lose keyboard focus while running Tux Racer, try setting \n"
        "# this to true." 
    );
    INIT_PARAM_BOOL( do_intro_animation, True, "# If false, then the introductory animation sequence will be skipped." );
    ...
    INIT_PARAM_BOOL( fullscreen, True, "# If true then the game will run in full-screen mode." );
    INIT_PARAM_INT( bpp_mode, 0,
    ...
    INIT_PARAM_BOOL( force_window_position, False ,
    ...
    INIT_PARAM_STRING( quit_key, "q escape" , "# Key binding for quitting a race" );
    INIT_PARAM_STRING( turn_left_key, "j left" , "# Key binding for turning left" );
    INIT_PARAM_STRING( turn_right_key, "l right" , "# Key binding for turning right" );
    INIT_PARAM_STRING( trick_modifier_key, "d" , "# Key binding for doing tricks" );
    INIT_PARAM_STRING( brake_key, "k space down" , "# Key binding for braking" );
    INIT_PARAM_STRING( paddle_key, "i up" , "# Key binding for paddling (on the ground) and flapping (in the air)" );
    INIT_PARAM_STRING( follow_view_key, "1" , "# Key binding for the \"Follow\" camera mode" );
    INIT_PARAM_STRING( behind_view_key, "2" , "# Key binding for the \"Behind\" camera mode" );
    INIT_PARAM_STRING( above_view_key, "3" , "# Key binding for the \"Above\" camera mode" );
    INIT_PARAM_INT( view_mode, 1 ,
    ...
    INIT_PARAM_STRING( screenshot_key, "=" , "# Key binding for taking a screenshot" );
    INIT_PARAM_STRING( pause_key, "p" , "# Key binding for pausing the game" );
    INIT_PARAM_STRING( reset_key, "backspace" , "# Key binding for resetting the player position" );
    INIT_PARAM_STRING( jump_key, "e" , "# Key binding for jumping" );
    INIT_PARAM_INT( joystick_paddle_button, 0 , 
        "# Joystick button for paddling (numbering starts at 0). \n"
        "# Set to -1 to disable." 
    );
    INIT_PARAM_INT( joystick_brake_button, 2 ,
        "# Joystick button for braking (numbering starts at 0). \n" 
        "# Set to -1 to disable." 
    );
    INIT_PARAM_INT( joystick_jump_button, 3 , "# Joystick button for jumping (numbering starts at 0)" );
    INIT_PARAM_INT( joystick_trick_button, 1 , "# Joystick button for doing tricks (numbering starts at 0)" );
    INIT_PARAM_INT( joystick_continue_button, 0 ,
        "# Joystick button for moving past intro, paused, and \n"
        "# game over screens (numbering starts at 0)" 
    );
    INIT_PARAM_INT( joystick_x_axis, 0 , "# Joystick axis to use for turning (numbering starts at 0)" );
    INIT_PARAM_INT( joystick_y_axis, 1 , "# Joystick axis to use for paddling/braking (numbering starts at 0)" );
    INIT_PARAM_INT( fov, 60 , "# [EXPERT] Sets the camera field-of-view" );
    INIT_PARAM_STRING( debug, "" , "# [EXPERT] Controls the Tux Racer debugging modes" );
    INIT_PARAM_INT( warning_level, 100 , "# [EXPERT] Controls the Tux Racer warning messages" );
    INIT_PARAM_INT( forward_clip_distance, 75 ,
    ...
    INIT_PARAM_BOOL( terrain_envmap, True ,
    ...
    INIT_PARAM_BOOL( disable_fog, False ,
    ...
    INIT_PARAM_INT( course_detail_level, 75 ,
    ...
    INIT_PARAM_BOOL( no_audio, False , "# If True, then audio in the game is completely disabled." );
    INIT_PARAM_BOOL( sound_enabled, True , "# Use this to turn sound effects on and off." );
    INIT_PARAM_BOOL( music_enabled, True , "# Use this to turn music on and off." );
    INIT_PARAM_INT( sound_volume, 127 , "# This controls the sound volume (valid range is 0-127)." );
    INIT_PARAM_INT( music_volume, 64 , "# This controls the music volume (valid range is 0-127)." );
    INIT_PARAM_INT( audio_freq_mode, 1 ,
    ...
    INIT_PARAM_INT( audio_format_mode, 1 ,
    ...
    INIT_PARAM_BOOL( audio_stereo, True , "# Audio will be played in stereo of true, and mono if false" );
    INIT_PARAM_INT( audio_buffer_size, 2048 ,
    ...
    INIT_PARAM_BOOL( track_marks, True , "# If true, then the players will leave track marks in the snow." );
    INIT_PARAM_BOOL( ui_snow, True , "# If true, then the ui screens will have falling snow." );
    INIT_PARAM_BOOL( write_diagnostic_log, False ,
...
{
#if defined(WIN32) 
    if (strlen(OLD_CONFIG_FILE) + 1 > (size_t)len ) { //---- kong ---- add size_t
        return 1;
    }
...
{
#if defined(WIN32) 
    if (strlen(CONFIG_DIR) + 1 > (size_t)len) { //---- kong ---- add size_t
        return 1;
    }
...
int get_config_file_name(char *buff, int len)
{
    ...
    if (strlen(buff) + strlen(CONFIG_FILE) + 2 > (size_t)len) { //---- kong ---- add size_t
        return 1;
    }
#if defined(WIN32) 
    strcat(buff, "\\");
...
void read_config_file()
{
    ...
    if (dir_exists(config_dir)) {
        if (file_exists(config_file)) {
            ...
            if (Tcl_EvalFile(g_game.tcl_interp, config_file) != TCL_OK) {
                handle_error(1, "error evalating %s: %s", config_file, Tcl_GetStringResult(g_game.tcl_interp));
            }
        }
        return;
    }
    ...
    if (Tcl_EvalFile(g_game.tcl_interp, config_file) != TCL_OK) {
        handle_error(1, "error evalating deprecated %s: %s", config_file, Tcl_GetStringResult(g_game.tcl_interp));
    } else {
...
void write_config_file()
{
    ...
    if (config_stream == NULL) {
        print_warning(CRITICAL_WARNING, "couldn't open %s for writing: %s", config_file, strerror(errno));
        return;
    }

    fprintf(config_stream, "# Tux Racer " VERSION " configuration file\n#\n");
...
static int get_param_cb (ClientData cd, Tcl_Interp *ip, int argc, char *argv[])
{
    int i;
    int num_params;
    struct param *parm;
    ...
    if (argc != 2) {
        Tcl_AppendResult(ip, argv[0], ": invalid number of arguments\n", "Usage: ", argv[0], " <parameter name>", (char*)0);
        return TCL_ERROR;
    } 
    ...
    if (parm == NULL || i == num_params) {
        Tcl_AppendResult(ip, argv[0], ": invalid parameter `", argv[1], "'", (char*)0);
        return TCL_ERROR;
    }
...
static int set_param_cb (ClientData cd, Tcl_Interp *ip, int argc, char *argv[]) 
{
    int i;
    int tmp_int;
    int num_params;
    ...
    if (argc != 3) {
        Tcl_AppendResult(ip, argv[0], ": invalid number of arguments\n", "Usage: ", argv[0], " <parameter name> <value>", (char*)0);
        return TCL_ERROR;
    } 
    ...
    case PARAM_CHAR:
        if (strlen(argv[2]) > 1) {
            Tcl_AppendResult(ip, "\n", argv[0], ": value for `", argv[1], "' must be a single character", (char*)0);
            return TCL_ERROR;
        }
    ...
    case PARAM_INT:
        if (Tcl_GetInt(ip, argv[2], &tmp_int) != TCL_OK) {
            Tcl_AppendResult(ip, "\n", argv[0], ": value for `", argv[1], "' must be an integer", (char*)0);
            return TCL_ERROR;
        }
    ...
    case PARAM_BOOL:
        if (Tcl_GetBoolean(ip, argv[2], &tmp_int) != TCL_OK) {
            Tcl_AppendResult(ip, "\n", argv[0], ": value for `", argv[1], "' must be a boolean", (char*)0);
            return TCL_ERROR;
        }
        check_assertion(tmp_int == 0 || tmp_int == 1, "invalid boolean value");
