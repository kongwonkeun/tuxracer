...
/* distance by which to push back far clip plane, to ensure that the fogging plane is drawn (m) */
...
const colour_t red        = { 1.0,  0.0,  0.0,  1.0 };
const colour_t green      = { 0.0,  1.0,  0.0,  1.0 };
const colour_t blue       = { 0.0,  0.0,  1.0,  1.0 };
const colour_t light_blue = { 0.5,  0.5,  0.8,  1.0 };
const colour_t black      = { 0.0,  0.0,  0.0,  1.0 };
...
void reshape(int w, int h)
{
    ...
    gluPerspective(getparam_fov(), (w / h), NEAR_CLIP_DIST, far_clip_dist);
...
{
    glDepthMask(GL_TRUE);
    glClearColor(ui_background_colour.r, ui_background_colour.g, ui_background_colour.b, ui_background_colour.a);
    glClearStencil(0);
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT | GL_STENCIL_BUFFER_BIT);
}

void set_material(colour_t diffuse_colour, colour_t specular_colour, scalar_t specular_exp)
{
    ...
    /* set standard colour */
    glColor4f(diffuse_colour.r, diffuse_colour.g, diffuse_colour.b, diffuse_colour.a);
} 

void draw_billboard(player_data_t* plyr, point_t center_pt, scalar_t width, scalar_t height, bool_t use_world_y_axis, point2d_t min_tex_coord, point2d_t max_tex_coord)
{
    point_t  pt;
    vector_t x_vec;
    vector_t y_vec;
    ...
    glNormal3f(z_vec.x, z_vec.y, z_vec.z);            glTexCoord2f(min_tex_coord.x, min_tex_coord.y); glVertex3f(pt.x, pt.y, pt.z);
    pt = move_point(pt, scale_vector( width, x_vec)); glTexCoord2f(max_tex_coord.x, min_tex_coord.y); glVertex3f(pt.x, pt.y, pt.z);
    pt = move_point(pt, scale_vector(height, y_vec)); glTexCoord2f(max_tex_coord.x, max_tex_coord.y); glVertex3f(pt.x, pt.y, pt.z);
    pt = move_point(pt, scale_vector(-width, x_vec)); glTexCoord2f(min_tex_coord.x, max_tex_coord.y); glVertex3f(pt.x, pt.y, pt.z);
    glEnd();
}
