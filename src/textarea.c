...
static void calc_text_region_dims(textarea_t *ta)
{
    ...
    ta->text_region_width  = ta->w - 2 * ta->border_width - 2 * ta->text_pad - ta->arrow_width;
    ta->text_region_height = ta->h - 2 * ta->border_width - 2 * ta->text_pad;
}
...
static void create_lines(textarea_t *ta)
{
    ...
    if (!get_font_binding(ta->font_binding, &font)) {
        print_warning(IMPORTANT_WARNING, "Couldn't get font for binding %s", ta->font_binding);
        return;
    }
    ...
    while (*cur_ptr != '\0') {
        ...
        if (old_space_ptr == NULL) {
            ...
            */
            cur_line = insert_list_elem(ta->line_list, cur_line, (list_elem_data_t)cur_ptr);
...
static void draw_text_lines(textarea_t *ta, font_t *font)
{
    ...
    glPushMatrix();
    {
        ...
        glTranslatef(ta->pos.x + ta->border_width + ta->text_pad , cur_y, 0);
...
void textarea_draw(textarea_t *ta)
{
    ...
    glRectf(ta->pos.x, ta->pos.y, ta->pos.x + ta->w, ta->pos.y + ta->h);
    glColor3dv((scalar_t*)&ta->background_colour);
    glRectf(ta->pos.x + ta->border_width, ta->pos.y + ta->border_width, ta->pos.x + ta->w - ta->border_width, ta->pos.y + ta->h - ta->border_width);

    glEnable(GL_TEXTURE_2D);
    if (!get_font_binding(ta->font_binding, &font)) {
        print_warning(IMPORTANT_WARNING, "couldn't get font object for binding %s", ta->font_binding);
    } else {
        draw_text_lines(ta, font);
    }
    if (ta->up_button != NULL)   button_draw(ta->up_button);
    if (ta->down_button != NULL) button_draw(ta->down_button);
}
...
static void update_button_positions(textarea_t *ta)
{
    ...
    button_set_position( 
        ta->up_button,
        make_point2d(
            ta->pos.x + ta->w - ta->border_width - button_get_width(ta->up_button),
            ta->pos.y + ta->h - ta->border_width - button_get_height(ta->up_button)
        ) 
    );
    button_set_position(
        ta->down_button,
        make_point2d(
            ta->pos.x + ta->w - ta->border_width - button_get_width(ta->down_button),
...
static void textarea_arrow_click_cb(button_t *button, void *userdata)
{
    ...
    if (button == ta->up_button) {
        ta->top_line = get_prev_list_elem(ta->line_list, ta->top_line);
    } else if (button == ta->down_button) {
        ta->top_line = get_next_list_elem(ta->line_list, ta->top_line);
    } else {
        check_assertion(0, "textarea arrow click callback called with unknown button");
    }
    update_button_enabled_states(ta);
    ui_set_dirty();
}

textarea_t* textarea_create(point2d_t pos, scalar_t w, scalar_t h, char *font_binding, char *text)
{
    textarea_t *ta;
    char *binding;
    point2d_t ll, ur;
    ...
    button_set_disabled_image(ta->up_button, binding, ll, ur, white);

    ll = make_point2d(34.0 / 64.0, 49.0 / 64.0);
    ur = make_point2d(61.0 / 64.0, 64.0 / 64.0);
    button_set_hilit_image(ta->up_button, binding, ll, ur, white);

    ll = make_point2d(2.0  / 64.0, 49.0 / 64.0);
    ur = make_point2d(29.0 / 64.0, 64.0 / 64.0);
    button_set_clicked_image(ta->up_button, binding, ll, ur, white);
    button_set_click_event_cb(ta->up_button, textarea_arrow_click_cb, ta);
    ...
    button_set_disabled_image(ta->down_button, binding, ll, ur, white);

    ll = make_point2d(34.0 / 64.0, 33.0 / 64.0);
    ur = make_point2d(61.0 / 64.0, 48.0 / 64.0);
    button_set_hilit_image(ta->down_button, binding, ll, ur, white);

    ll = make_point2d(2.0  / 64.0, 33.0 / 64.0);
    ur = make_point2d(29.0 / 64.0, 48.0 / 64.0);
    button_set_clicked_image(ta->down_button, binding, ll, ur, white);
    button_set_click_event_cb(ta->down_button, textarea_arrow_click_cb, ta);
...
void textarea_delete(textarea_t *ta)
{
    ...
    if (ta->up_button != NULL)   button_delete(ta->up_button);
    if (ta->down_button != NULL) button_delete(ta->down_button);
    if (ta->line_list != NULL)   del_list(ta->line_list);
    if (ta->text_lines != NULL)  free(ta->text_lines);
    if (ta->text_orig != NULL)   free(ta->text_orig);
    free(ta);
}
...
{
    check_assertion(ta != NULL, "ta is NULL");
    if (ta->up_button)   button_set_active(ta->up_button, active);
    if (ta->down_button) button_set_active(ta->down_button, active);
    ta->active = active;
    if (!active) {
        textarea_set_visible(ta, False);
    }
