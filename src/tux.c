...
void adjust_tux_joints(scalar_t turnFact, bool_t isBraking, scalar_t paddling_factor, scalar_t speed, vector_t net_force, scalar_t flap_factor)
{
...
void load_tux()
{
    ...
    if (_getcwd(cwd, BUFF_LEN) == NULL) {
        handle_system_error(1, "getcwd failed");
    }
    if (_chdir(getparam_data_dir()) != 0) {
        handle_system_error(1, "can't find the tuxracer data directory.\n"
            "please check the value of `data_dir' in options and set it to the location\n"
            "where you installed the tuxracer-data files\n\n"
            "couldn't chdir to %s",
            getparam_data_dir()
        );
    } 
    if (Tcl_EvalFile(g_game.tcl_interp, "tux.tcl") == TCL_ERROR) {
        handle_error(1, "error evaluating %s/tux.tcl: %s\n"
            "please check the value of `data_dir' in options and make sure\n"
            "it points to the location of the latest version of the tuxracer-data files", 
            getparam_data_dir(),
...
{
    if (argc != 2) {
        Tcl_AppendResult(ip, argv[0], ": invalid number of arguments\n", "Usage: ", argv[0], " <head joint>", (char*)0);
        return TCL_ERROR;
    } 
...
static int neck_cb(ClientData cd, Tcl_Interp *ip, int argc, char *argv[]) 
{
    if (argc != 2) {
        Tcl_AppendResult(ip, argv[0], ": invalid number of arguments\n", "Usage: ", argv[0], " <neck joint>", (char*)0);
        return TCL_ERROR;
    } 
...
static int root_node_cb(ClientData cd, Tcl_Interp *ip, int argc, char *argv[]) 
{
    if (argc != 2) {
        Tcl_AppendResult(ip, argv[0], ": invalid number of arguments\n", "Usage: ", argv[0], " <root node>", (char*)0);
        return TCL_ERROR;
    } 
...
static int left_shoulder_cb(ClientData cd, Tcl_Interp *ip, int argc, char *argv[]) 
{
    if (argc != 2) {
        Tcl_AppendResult(ip, argv[0], ": invalid number of arguments\n", "Usage: ", argv[0], " <left shoulder joint>", (char*)0);
        return TCL_ERROR;
    }
...
static int right_shoulder_cb(ClientData cd, Tcl_Interp *ip, int argc, char *argv[]) 
{
    if (argc != 2) {
        Tcl_AppendResult(ip, argv[0], ": invalid number of arguments\n", "Usage: ", argv[0], " <right shoulder joint>", (char*)0);
        return TCL_ERROR;
    } 
...
static int left_hip_cb(ClientData cd, Tcl_Interp *ip, int argc, char *argv[]) 
{
    if (argc != 2) {
        Tcl_AppendResult(ip, argv[0], ": invalid number of arguments\n", "Usage: ", argv[0], " <left hip joint>", (char*)0);
        return TCL_ERROR;
    } 
...
static int right_hip_cb(ClientData cd, Tcl_Interp *ip, int argc, char *argv[]) 
{
    if (argc != 2) {
        Tcl_AppendResult(ip, argv[0], ": invalid number of arguments\n", "Usage: ", argv[0], " <right hip joint>", (char*)0);
        return TCL_ERROR;
    } 
...
static int left_knee_cb(ClientData cd, Tcl_Interp *ip, int argc, char *argv[]) 
{
    if (argc != 2) {
        Tcl_AppendResult(ip, argv[0], ": invalid number of arguments\n", "Usage: ", argv[0], " <left knee joint>", (char*)0);
        return TCL_ERROR;
    } 
...
static int right_knee_cb(ClientData cd, Tcl_Interp *ip, int argc, char *argv[]) 
{
    if (argc != 2) {
        Tcl_AppendResult(ip, argv[0], ": invalid number of arguments\n", "Usage: ", argv[0], " <right knee joint>", (char*)0);
        return TCL_ERROR;
    } 
...
static int left_ankle_cb(ClientData cd, Tcl_Interp *ip, int argc, char *argv[]) 
{
    if (argc != 2) {
        Tcl_AppendResult(ip, argv[0], ": invalid number of arguments\n", "Usage: ", argv[0], " <left ankle joint>", (char*)0);
        return TCL_ERROR;
    } 
...
static int right_ankle_cb(ClientData cd, Tcl_Interp *ip, int argc, char *argv[]) 
{
    if (argc != 2) {
        Tcl_AppendResult(ip, argv[0], ": invalid number of arguments\n", "Usage: ", argv[0], " <right ankle joint>", (char*)0);
        return TCL_ERROR;
    }
...
static int tail_cb(ClientData cd, Tcl_Interp *ip, int argc, char *argv[]) 
{
    if (argc != 2) {
        Tcl_AppendResult(ip, argv[0], ": invalid number of arguments\n", "Usage: ", argv[0], " <tail joint>", (char*)0);
        return TCL_ERROR;
    } 
