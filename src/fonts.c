...
static bool_t del_font(char *fontname)
{
    ...
    if (del_hash_entry(font_table, fontname, (hash_entry_t*)(&fontnode))) {
        check_assertion(fontnode->ref_count == 0, "trying to delete font with non-zero reference count");
        delete_tex_font_metrics(fontnode->tfm);
        fontnode->tex->ref_count -= 1;
        free(fontnode);
        return True;
...
bool_t bind_font(char *binding, char *fontname, scalar_t size, colour_t colour)
{
    font_node_t *fontnode;
    font_t *font;
...
static scalar_t get_scale_factor(font_t *font) 
{
    ...
    get_tex_font_string_bbox(font->node->tfm, empty_string, &dummy1, &max_ascent, &dummy2);
...
void get_font_metrics(font_t *font, char *string, int *width, int *max_ascent, int *max_descent)
{
    scalar_t scale_fact = get_scale_factor(font);
    get_tex_font_string_bbox(font->node->tfm, string, width, max_ascent, max_descent);
    *width *= scale_fact;
    *max_ascent *= scale_fact;
    *max_descent *= scale_fact;
}

static int load_font_cb(ClientData cd, Tcl_Interp *ip, int argc, char *argv[]) 
{
    bool_t error = False;
    char *fontname = NULL;
    char *filename = NULL;
    ...
    while (!error && argc > 0) {
        if (strcmp("-name", *argv) == 0) {
            ...
            print_warning(TCL_WARNING, "tux_load_font: unrecognized parameter `%s'", *argv);
        }
        NEXT_ARG;
    }
    ...
    if (error) {
        ...
        Tcl_AppendResult( ip, "\nusage: tux_load_font -name <name> -file <file> -texture <name> ", (char*)0);
        return TCL_ERROR;
    }
...
static int bind_font_cb(ClientData cd, Tcl_Interp *ip, int argc, char *argv[]) 
{
    double tmp_dbl;
    bool_t error = False;
    char *binding = NULL;
    ...
    while (!error && argc > 0) {
        if (strcmp("-binding", *argv) == 0) {
            ...
            print_warning(TCL_WARNING, "tux_bind_font: unrecognized parameter `%s'", *argv);
        }
        NEXT_ARG;
    }
    /* sanity check on values */
    ...
    if (error) {
        ...
        Tcl_AppendResult(ip, "\nusage: tux_bind_font -binding <name> -font <name> [-size <height>] ", (char*)0);
        return TCL_ERROR;
    }
