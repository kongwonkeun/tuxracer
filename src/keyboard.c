...
int add_keymap_entry( game_mode_t mode, keymap_class_t keymap_class, char *keys, key_func_t key_func, key_cb_t key_cb )
{
    ...
    for (i=0; i<num_keymap_entries; i++) {
        if ( keymap[i].mode == mode || keymap[i].mode == ALL_MODES ) {
            ...
            case CONFIGURABLE_KEY:
                ...
                {
                    fprintf( stderr, "Tux Racer warning: key specification '%s' is unrecognized; using '%s' instead.\n", keys, keymap[i].keys );
                    if ( ! insert_keytable_entries( keymap[i].keys, keymap[i].key_cb ) )
                    {
...
static void keyboard_handler( unsigned int key, bool_t special, bool_t release, int x, int y )
{
