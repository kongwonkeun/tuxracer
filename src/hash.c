...
void add_hash_entry(hash_table_t table, char* name, hash_entry_t entry)
{
    ...
    hash_entry = Tcl_CreateHashEntry((Tcl_HashTable*)table, name, &newEntry);
    check_assertion(newEntry, "hash entry already in table");
    Tcl_SetHashValue(hash_entry, entry);
}

bool_t get_hash_entry(hash_table_t table, char *name, hash_entry_t *result)
{
    Tcl_HashEntry *entry;
...
bool_t del_hash_entry(hash_table_t table, char *name, hash_entry_t *result)
{
    Tcl_HashEntry *entry;
...
void begin_hash_scan(hash_table_t table, hash_search_t *sptr)
{
    hash_scan_t *scanPtr;
...
bool_t next_hash_entry(hash_search_t sptr, char** key, hash_entry_t *value)
{
    hash_scan_t *scanPtr;
