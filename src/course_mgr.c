...
{
    check_assertion(initialized == False, "attempt to initialize course manager twice");
    initialized = True;
...
list_elem_t get_last_complete_cup_for_event( event_data_t *event_data )
{
    ...
    if ( get_last_completed_cup( g_game.player[local_player()].name, get_event_name( event_data ), g_game.difficulty, &last_cup ) ) {
        cup = get_event_cup_by_name( event_data, last_cup );

        if ( cup == NULL ) {
            print_warning( IMPORTANT_WARNING, "Couldn't find saved cup `%s'", last_cup );
        }
        return cup;
    } else {
        return NULL;
...
bool_t is_cup_complete( event_data_t *event_data, list_elem_t cup )
{
    ...
    list_elem_t last_complete_cup = get_last_complete_cup_for_event( event_data );
...
bool_t is_cup_first_incomplete_cup( event_data_t *event_data, list_elem_t cup )
{
...
int compare_race_positions( cup_data_t *cup_data, list_elem_t race1, list_elem_t race2 )
{
    int incr = 1;
...
open_course_data_t* create_open_course_data( Tcl_Interp *ip, char *string, char **err_msg )
{
    char **argv = NULL;
    char **orig_argv = NULL;
    int argc = 0;
    ...
    while ( *argv != NULL ) {
        if ( strcmp( *argv, "-course" ) == 0 ) {
            ...
            if ( *argv == NULL ) {
                par_time = 120.0;
                print_warning( PEDANTIC_WARNING, "No data supplied for -par_time in open course data. Using %g seconds.", par_time );
            } else if ( Tcl_GetDouble( ip, *argv, &par_time ) != TCL_OK ) {
                *err_msg = "Invalid value for -par_time in open course data";
                goto bail_open_course_data;
            }
        } else {
            sprintf( err_buff, "unrecognized option `%s' in open course data", *argv );
            *err_msg = err_buff;
            goto bail_open_course_data;
        }
        NEXT_ARG;
...
static int open_courses_cb( ClientData cd, Tcl_Interp *ip, int argc, char **argv )
{
    char *err_msg;
    char **list = NULL;
    int num_courses;
    ...
    check_assertion( initialized, "course_mgr module not initialized" );
    ...
    for ( i=0; i<num_courses; i++ ) {
        ...
        last_elem = insert_list_elem( open_course_list, last_elem, (list_elem_data_t) data );
    }
...
bail_open_courses:
    ...
    Tcl_AppendResult( ip, "Error in call to tux_open_courses: ", err_msg, "\n", "Usage: tux_open_courses { list of open courses }", (NULL) );
    return TCL_ERROR;
}

static race_data_t* create_race_data ( Tcl_Interp *ip, char *string, char **err_msg )
{
    char **argv = NULL;
    char **orig_argv = NULL;
    int argc = 0;
    ...
    while ( *argv != NULL ) {
        if ( strcmp( *argv, "-course" ) == 0 ) {
            ...
            if ( get_tcl_int_tuple( ip, *argv, herring_req, sizeof(herring_req)/sizeof(herring_req[0]) ) == TCL_ERROR ) {
                *err_msg = "Value for -herring is not a list or has the wrong number of elements";
                goto bail_race_data;
            }
            ...
            if ( get_tcl_tuple( ip, *argv, time_req, sizeof(time_req)/sizeof(time_req[0]) ) == TCL_ERROR ) {
                *err_msg = "Value for -time is not a list or hsa the wrong number of elements";
                goto bail_race_data;
            }
           ...
           if ( *argv == NULL ) {
            ...
            if ( get_tcl_tuple( ip, *argv, score_req, sizeof(score_req)/sizeof(score_req[0]) ) == TCL_ERROR ) {
                *err_msg = "Value for -score is not a list or has the wrong number of elements";
                goto bail_race_data;
            }
            ...
            for ( i=0; i<RACE_CONDITIONS_NUM_CONDITIONS; i++ ) {
                if ( string_cmp_no_case( race_condition_names[i], *argv ) == 0 ) {
                    break;
                }
            }
            ...
            sprintf( err_buff, "unrecognized option `%s' in race data", *argv );
            *err_msg = err_buff;
            goto bail_race_data;
        }
        NEXT_ARG;
    ...
    if ( !herring_req_init || !time_req_init || !score_req_init ) {
...
static cup_data_t* create_cup_data( Tcl_Interp *ip, char *string, char **err_msg )
{
    char **argv = NULL;
    char **orig_argv = NULL;
    int argc = 0;
    ...
    while ( *argv != NULL ) {
        if ( strcmp( *argv, "-name" ) == 0 ) {
            ...
            for ( i=0; i<num_races; i++) {
                ...
                last_race = insert_list_elem( race_list, last_race, (list_elem_data_t) race_data );
            }
...
static event_data_t* create_event_data( Tcl_Interp *ip, char *string, char **err_msg )
{
    char **orig_argv = NULL;
    char **argv = NULL;
    int argc = 0;
    ...
    while ( *argv != NULL ) {
        if ( strcmp( *argv, "-name" ) == 0 ) {
            ...
            for ( i=0; i<num_cups; i++ ) {
                ...
                last_cup = insert_list_elem( cup_list, last_cup, (list_elem_data_t) cup_data );
            }
...
bail_event_data:
    ...
    if ( cup_list ) {
        ...
        while ( last_cup != NULL ) {
            cup_data_t *data;
            data = (cup_data_t*) delete_list_elem( cup_list, last_cup );
            free( data );
            last_cup = get_list_tail( cup_list );
        }
        del_list( cup_list );
...
static int events_cb(ClientData cd, Tcl_Interp* ip, int argc, char** argv)
{
    char *err_msg;
    char **list = NULL;
    int num_events;
    ...
    if (last_event != NULL) {
        err_msg = "tux_events has already been called; it can only be called once.";
        goto bail_events;
    }
    for (i = 0; i < num_events; i++) {
        ...
        last_event = insert_list_elem(event_list, last_event, (list_elem_data_t)data);
    }
    Tcl_Free((char*)list);
    list = NULL;
    return TCL_OK;
...
bail_events:
    ...
    if (event_list != NULL) {
        ...
        while (last_event != NULL) {
            event_data_t *data;
            data = (event_data_t*)delete_list_elem(event_list, last_event);
            free(data);
            last_event = get_list_tail(event_list);
        }
    }
    Tcl_AppendResult(ip, "Error in call to tux_events: ", err_msg, "\n", "Usage: tux_events { list of event data }", (NULL));
    return TCL_ERROR;
}

static int get_race_conditions_cb(ClientData cd, Tcl_Interp* ip, int argc, char** argv)
{
    char *err_msg;
    Tcl_Obj *result;
    ...
    //---- kong ---- strlen to (int)strlen
    result = Tcl_NewStringObj(race_condition_names[g_game.race.conditions], (int)strlen(race_condition_names[g_game.race.conditions]));;
    Tcl_SetObjResult(ip, result);
    return TCL_OK;

bail_race_conditions:

    Tcl_AppendResult(ip, "Error in call to tux_get_race_conditions: ", err_msg, "\n", "Usage: tux_get_race_conditions", (NULL));
    return TCL_ERROR;
}
...
void register_course_manager_callbacks(Tcl_Interp *ip)
{
    ...
    Tcl_CreateCommand(ip, "tux_get_race_conditions", get_race_conditions_cb, 0, 0);
}
