...
void setup_course_lighting()
{
    ...
    for (i=0; i<NUM_COURSE_LIGHTS; i++) {
        ...
        glLightfv( GL_LIGHT0 + i, GL_SPOT_DIRECTION, course_lights[i].spot_direction );
        glLightf( GL_LIGHT0 + i, GL_SPOT_EXPONENT, course_lights[i].spot_exponent );
        glLightf( GL_LIGHT0 + i, GL_SPOT_CUTOFF, course_lights[i].spot_cutoff );
        glLightf( GL_LIGHT0 + i, GL_CONSTANT_ATTENUATION, course_lights[i].constant_attenuation );
        glLightf( GL_LIGHT0 + i, GL_LINEAR_ATTENUATION, course_lights[i].linear_attenuation );
        glLightf( GL_LIGHT0 + i, GL_QUADRATIC_ATTENUATION, course_lights[i].quadratic_attenuation );
    }
}

static int course_light_cb (ClientData cd, Tcl_Interp *ip, int argc, char **argv) 
{
    int light_num;
    scalar_t tmp_arr[4];
    double tmp_dbl;
    ...
    while ( !error && argc > 0 ) {
        if ( strcmp( "-on", *argv ) == 0 ) {
            ...
            copy_to_glfloat_array( course_lights[light_num].ambient, tmp_arr, 4 );
            ...
            copy_to_glfloat_array( course_lights[light_num].diffuse, tmp_arr, 4 );
            ...
            copy_to_glfloat_array( course_lights[light_num].specular, tmp_arr, 4 );
            ...
            copy_to_glfloat_array( course_lights[light_num].position, tmp_arr, 4 );
            ...
            copy_to_glfloat_array( course_lights[light_num].spot_direction, tmp_arr, 3 );
            ...
            print_warning( TCL_WARNING, "tux_course_light: unrecognized parameter `%s'", *argv );
        }
        NEXT_ARG;
    }
