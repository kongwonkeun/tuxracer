...
/* how long to make the flat part at the bottom of the course as a fraction of the total length of the course */
        ...
        for (x = 0; x < nx; x++) {
            ...
            /* the terrain is meshed as follows:
               ...
               ...+-+-+-+-+...              V
                  ...
                  +-+-+-+-+
                     ...
               so there are two types of vertices:
               those surrounded by four triangles (x + y is odd), 
               and those surrounded by eight (x + y is even).
            */
...
void draw_sky(point_t pos)
{
    ...
    set_gl_options(SKY);
    if (!(
        get_texture_binding("sky_front",  &texture_id[0]) && 
...
void draw_trees() 
{
    ...
    for (i = 0; i < numTrees; i++) {
        ...
        glPushMatrix();
        glTranslatef(treeLocs[i].ray.pt.x, treeLocs[i].ray.pt.y, treeLocs[i].ray.pt.z);
        ...
        glBegin(GL_QUADS);
        glTexCoord2f(0.0, 0.0); glVertex3f(-treeRadius, 0.0, 0.0);
        glTexCoord2f(1.0, 0.0); glVertex3f( treeRadius, 0.0, 0.0);
        glTexCoord2f(1.0, 1.0); glVertex3f( treeRadius, treeHeight, 0.0);
        glTexCoord2f(0.0, 1.0); glVertex3f(-treeRadius, treeHeight, 0.0);

        if (!clip_course || eye_pt.z - treeLocs[i].ray.pt.z < fwd_tree_detail_limit) {
            glTexCoord2f(0.0, 0.0); glVertex3f(0.0, 0.0, -treeRadius);
            glTexCoord2f(1.0, 0.0); glVertex3f(0.0, 0.0,  treeRadius);
            glTexCoord2f(1.0, 1.0); glVertex3f(0.0, treeHeight,  treeRadius);
            glTexCoord2f(0.0, 1.0); glVertex3f(0.0, treeHeight, -treeRadius);
        }
        glEnd();
        glPopMatrix();
    } 
    ...
    for (i = 0; i < numItems; i++) {
        ...
        glPushMatrix();
        glTranslatef(itemLocs[i].ray.pt.x, itemLocs[i].ray.pt.y, itemLocs[i].ray.pt.z);
        ...
        glBegin(GL_QUADS);
        glTexCoord2f(0.0, 0.0); glVertex3f(-itemRadius*normal.z, 0.0,  itemRadius*normal.x);
        glTexCoord2f(1.0, 0.0); glVertex3f( itemRadius*normal.z, 0.0, -itemRadius*normal.x);
        glTexCoord2f(1.0, 1.0); glVertex3f( itemRadius*normal.z, itemHeight, -itemRadius*normal.x);
        glTexCoord2f(0.0, 1.0); glVertex3f(-itemRadius*normal.z, itemHeight,  itemRadius*normal.x);
        glEnd();
...
void draw_fog_plane()
{
    ...
    bottom_plane.d = -(pt.x * bottom_plane.nml.x + pt.y * bottom_plane.nml.y + pt.z * bottom_plane.nml.z);
    */
    ...
    if (!intersect_planes(bottom_plane, far_clip_plane, left_clip_plane,  &left_pt))   return;
    if (!intersect_planes(bottom_plane, far_clip_plane, right_clip_plane, &right_pt))  return;
    if (!intersect_planes(top_plane, far_clip_plane, left_clip_plane,  &top_left_pt))  return;
    if (!intersect_planes(top_plane, far_clip_plane, right_clip_plane, &top_right_pt)) return;
    if (!intersect_planes(bottom_clip_plane, far_clip_plane, left_clip_plane,  &bottom_left_pt))  return;
    if (!intersect_planes(bottom_clip_plane, far_clip_plane, right_clip_plane, &bottom_right_pt)) return;
    ...
    pt = move_point(top_left_pt,  left_vec);  glVertex3f(pt.x, pt.y, pt.z);
    pt = move_point(top_right_pt, right_vec); glVertex3f(pt.x, pt.y, pt.z);

    glColor4f(fog_colour[0], fog_colour[1], fog_colour[2], 0.0);
    pt = move_point(top_left_pt,  scale_vector(3.0, left_vec));  glVertex3f(pt.x, pt.y, pt.z);
    pt = move_point(top_right_pt, scale_vector(3.0, right_vec)); glVertex3f(pt.x, pt.y, pt.z);
    glEnd();
}
