...
static void update_button_enabled_states()
{
    ...
    if (is_cup_complete(event_data, cur_cup) || is_cup_first_incomplete_cup(event_data, cur_cup)) {
...
static void set_cur_cup_to_first_incomplete(event_data_t *event_data, list_t cup_list)
{
...
static void event_listbox_item_change_cb(listbox_t *listbox, void *userdata)
{
    ...
    listbox_set_item_list(cup_listbox, cup_list, cup_list_elem_to_string_func);
...
static void set_widget_positions_and_draw_decorations()
{
    ...
    button_set_position(continue_btn, make_point2d(x_org + 329 - button_get_width(continue_btn) / 2.0, 42));
    listbox_set_position(cup_listbox, make_point2d(x_org + 52, y_org + 103));
    listbox_set_position(event_listbox, make_point2d(x_org + 52, y_org + 193));

    /* Draw other stuff */
    /* Event & cup icons */
    if (!get_texture_binding(get_event_icon_texture_binding(event_data), &texobj)) {
        texobj = 0;
    }
    ...
    glBegin( GL_QUADS );
    {
        ...
        glTexCoord2f(tll.x, tll.y); glVertex2f(ll.x, ll.y);
        glTexCoord2f(tur.x, tll.y); glVertex2f(ur.x, ll.y);
        glTexCoord2f(tur.x, tur.y); glVertex2f(ur.x, ur.y);
        glTexCoord2f(tll.x, tur.y); glVertex2f(ll.x, ur.y);
    }
    glEnd();
    ...
    if (!get_font_binding("event_and_cup_label", &font)) {
        ...
        glPushMatrix();
        {
            glTranslatef(x_org, y_org + 193 + 44 + 5 + desc, 0);
            draw_string(font, string);
        }
        glPopMatrix();
        ...
        glPushMatrix();
        {
            glTranslatef(x_org, y_org + 103 + 44 + 5 + desc, 0);
            draw_string(font, string);
        }
        glPopMatrix();
    }
    ...
    if (!get_font_binding("cup_status", &font)) {
        print_warning(IMPORTANT_WARNING, "couldn't get font for binding cup_status");
    } else {
        bind_font_texture(font);
        get_font_metrics(font, string, &text_width, &asc, &desc);
        ...
        glPushMatrix();
        {
            glTranslatef(x_org + box_width / 2.0 - text_width / 2.0, y_org + 70, 0);
            draw_string(font, string);
        }
        glPopMatrix();
    }
...
static void event_select_init(void)
{
    ...
    /* continue button */
    continue_btn = button_create(dummy_pos, 150, 40, "button_label", "Continue");
    ...
    /* event listbox */
    event_listbox = listbox_create(dummy_pos, 460 - 52, 44, "listbox_item", event_list, event_list_elem_to_string_func);
    listbox_set_current_item(event_listbox, cur_event);
    listbox_set_item_change_event_cb(event_listbox, event_listbox_item_change_cb, NULL);
    listbox_set_visible(event_listbox, True);

    /* cup listbox */
    cup_listbox = listbox_create(dummy_pos, 460 - 52, 44, "listbox_item", cup_list, cup_list_elem_to_string_func);
    listbox_set_current_item(cup_listbox, cur_cup);
    listbox_set_item_change_event_cb(cup_listbox, cup_listbox_item_change_cb, NULL);
...
START_KEYBOARD_CB(event_select_key_cb)
{
    ...
    if (special) {
        switch(key) {
        case WSK_LEFT:  listbox_goto_prev_item(event_listbox); break;
        case WSK_RIGHT: listbox_goto_next_item(event_listbox); break;
        case WSK_DOWN:  listbox_goto_next_item(cup_listbox);   break;
        case WSK_UP:    listbox_goto_prev_item(cup_listbox);   break;
        }
    } else {
        switch (key) {
        case 13: /* Enter */
...
{
    int status = 0;
    status |= add_keymap_entry(EVENT_SELECT, DEFAULT_CALLBACK, NULL, NULL, event_select_key_cb);
    check_assertion(status == 0, "out of keymap entries");
    register_loop_funcs(EVENT_SELECT, event_select_init, event_select_loop, event_select_term);
}
