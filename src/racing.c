...
void racing_init(void) 
{
    ...
    /* We need to reset controls here since callbacks won't have been called in paused mode.
       This results in duplication between this code and init_physical_simulation. */
...
void calc_jump_amt(scalar_t time_step)
{
    ...
    if (plyr->control.jump_charging) {
        plyr->control.jump_amt = min(MAX_JUMP_AMT, g_game.time - charge_start_time);
    } else if (plyr->control.jumping) {
        plyr->control.jump_amt *= (1.0 - (g_game.time - plyr->control.jump_start_time) / JUMP_FORCE_DURATION);
    } else {
...
void racing_loop(scalar_t time_step)
{
    ...
    airborne = (bool_t)(plyr->pos.y > (find_y_coord(plyr->pos.x, plyr->pos.z) + JUMP_MAX_START_HEIGHT));
    ...
    if (is_joystick_active()) {
        ...
        if (getparam_joystick_brake_button() >= 0) {
            joy_braking = is_joystick_button_down(getparam_joystick_brake_button());
        } 
        if (!joy_braking) {
            joy_braking = (bool_t)(joy_y > 0.5);
        }
        if (getparam_joystick_paddle_button() >= 0) {
            joy_paddling = is_joystick_button_down(getparam_joystick_paddle_button());
        }
        if (!joy_paddling) {
            joy_paddling = (bool_t)(joy_y < -0.5);
        }
        if (getparam_joystick_jump_button() >= 0) {
            joy_charging = is_joystick_button_down(getparam_joystick_jump_button());
        }

        if (getparam_joystick_trick_button() >= 0) {
            joy_tricks = is_joystick_button_down(getparam_joystick_trick_button());
        }
    }
    ...
    if (airborne) {
        ...
        if (trick_modifier || joy_tricks) {
            if (left_turn  || joy_left_turn)  plyr->control.barrel_roll_left  = True;
            if (right_turn || joy_right_turn) plyr->control.barrel_roll_right = True;
            if (paddling   || joy_paddling)   plyr->control.front_flip = True;
            if (plyr->control.is_braking)     plyr->control.back_flip  = True;
        }
    } else {
        get_surface_type(plyr->pos.x, plyr->pos.z, terrain_weights);
        if (terrain_weights[Snow] > 0) new_terrain |= (1 << Snow);
        if (terrain_weights[Rock] > 0) new_terrain |= (1 << Rock);
        if (terrain_weights[Ice] > 0)  new_terrain |= (1 << Ice);
    }

    /* Jumping */
    calc_jump_amt(time_step);

    if ((charging || joy_charging) && !plyr->control.jump_charging && !plyr->control.jumping) {
    ...
    if ((left_turn || joy_left_turn) ^ (right_turn || joy_right_turn)) {
        ...
        plyr->control.turn_animation += (turning_left ? -1 : 1) * 0.15 * time_step / 0.05;
        plyr->control.turn_animation =  min(1.0, max(-1.0, plyr->control.turn_animation));
    } else {
        plyr->control.turn_fact = 0;
        /* Decay turn animation */
        if (time_step < ROLL_DECAY_TIME_CONSTANT) {
            plyr->control.turn_animation *= 1.0 - time_step / ROLL_DECAY_TIME_CONSTANT;
        } else {
            plyr->control.turn_animation = 0.0;
        }
    }
    ...
    /* Play sliding sound */
    slide_volume = min((((pow(plyr->control.turn_fact, 2) * 128)) + (plyr->control.is_braking? 128 : 0) + (plyr->control.jumping? 128 : 0) + 20) * (speed / 10), 128 );
    ...
    if (plyr->control.barrel_roll_left || plyr->control.barrel_roll_right) {
        plyr->control.barrel_roll_factor += (plyr->control.barrel_roll_left ? -1 : 1) * 0.15 * time_step / 0.05;
        if ((plyr->control.barrel_roll_factor > 1) || (plyr->control.barrel_roll_factor < -1)) {
    ...
    if (plyr->control.front_flip || plyr->control.back_flip) {
        plyr->control.flip_factor += (plyr->control.back_flip ? -1 : 1 ) * 0.15 * time_step / 0.05;
        if ((plyr->control.flip_factor  > 1) || (plyr->control.flip_factor  < -1) ) {
    ...
    setup_view_frustum(plyr, NEAR_CLIP_DIST, getparam_forward_clip_distance());
...
{
    int status = 0;
    status |= add_keymap_entry(RACING, CONFIGURABLE_KEY, "escape", getparam_quit_key,           quit_racing_cb);
    status |= add_keymap_entry(RACING, CONFIGURABLE_KEY, "j",      getparam_turn_left_key,      turn_left_cb);
    status |= add_keymap_entry(RACING, CONFIGURABLE_KEY, "l",      getparam_turn_right_key,     turn_right_cb);
    status |= add_keymap_entry(RACING, CONFIGURABLE_KEY, "d",      getparam_trick_modifier_key, trick_modifier_cb);
    status |= add_keymap_entry(RACING, CONFIGURABLE_KEY, "space",  getparam_brake_key,          brake_cb);
    status |= add_keymap_entry(RACING, CONFIGURABLE_KEY, "k",      getparam_paddle_key,         paddle_cb);
    status |= add_keymap_entry(RACING, CONFIGURABLE_KEY, "1",      getparam_behind_view_key,    behind_view_cb);
    status |= add_keymap_entry(RACING, CONFIGURABLE_KEY, "2",      getparam_follow_view_key,    follow_view_cb);
    status |= add_keymap_entry(RACING, CONFIGURABLE_KEY, "3",      getparam_above_view_key,     above_view_cb);
    status |= add_keymap_entry(RACING, CONFIGURABLE_KEY, "=",      getparam_screenshot_key,     screenshot_cb);
    status |= add_keymap_entry(RACING, CONFIGURABLE_KEY, "p",      getparam_pause_key,          pause_cb);
    status |= add_keymap_entry(RACING, CONFIGURABLE_KEY, "r",      getparam_reset_key,          reset_cb);
    status |= add_keymap_entry(RACING, CONFIGURABLE_KEY, "i",      getparam_jump_key,           jump_cb);
