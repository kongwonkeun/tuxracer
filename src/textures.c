...
{
    return (bool_t)(load_texture(binding, filename, 1) && bind_texture(binding, binding));
}
...
int get_min_filter()
{
    ...
    default: return GL_LINEAR_MIPMAP_NEAREST;
    }
}
...
bool_t load_texture(char *texname, char *filename, int repeatable)
{
    ...
    if (texImage == NULL) {
        print_warning(IMPORTANT_WARNING, "couldn't load image %s", filename);
        return False;
    }
    ...
    glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, get_min_filter());

    /* check if we need to scale image */
    glGetIntegerv(GL_MAX_TEXTURE_SIZE, &max_texture_size);
    if (texImage->sizeX > max_texture_size || texImage->sizeY > max_texture_size) {
        char *newdata = (char*)malloc(texImage->sizeZ * max_texture_size * max_texture_size);
        check_assertion(newdata != NULL, "out of memory");
        /* in case of large- or small-aspect ratio textures, this could end up using *more* space... oh well. */
        gluScaleImage(
            texImage->sizeZ == 3 ? GL_RGB : GL_RGBA, texImage->sizeX, texImage->sizeY,
            GL_UNSIGNED_BYTE, texImage->data, max_texture_size, max_texture_size, 
    ...
    gluBuild2DMipmaps( 
        GL_TEXTURE_2D, 
        texImage->sizeZ, texImage->sizeX, texImage->sizeY, 
        texImage->sizeZ == 3 ? GL_RGB : GL_RGBA,
        GL_UNSIGNED_BYTE, texImage->data 
    );

    //printf("---- LOAD:%s:%d:%x:%x:%x ----\n", texname, tex->texture_id, texImage->sizeZ, texImage->sizeX, texImage->sizeY); //---- kong ----
...
bool_t del_texture(char *texname)
{
    ...
    if (del_hash_entry(texture_table, texname, (hash_entry_t*)(&tex))) {
        check_assertion(tex->ref_count == 0, "trying to delete texture with non-zero reference count");
        glDeleteTextures(1, &(tex->texture_id));
        free(tex);
        return True;
    }
...
static int load_texture_cb(ClientData cd, Tcl_Interp *ip, int argc, char *argv[])
{
    int repeatable = 1;

    if ((argc != 3) && (argc != 4)) {
        Tcl_AppendResult(ip, argv[0], ": invalid number of arguments\n", "usage: ", argv[0], "<texture name> <image file>", " [repeatable]", (char*)0);
        return TCL_ERROR;
    } 
    if ((argc == 4) && (Tcl_GetInt(ip, argv[3], &repeatable) != TCL_OK)) {
        Tcl_AppendResult(ip, argv[0], ": invalid repeatable flag", (char*)0);
        return TCL_ERROR;
    } 
    if (!load_texture(argv[1], argv[2], repeatable)) {
        Tcl_AppendResult(ip, argv[0], ": could not load texture ", argv[2], (char*)0);
        return TCL_ERROR;
    }
    return TCL_OK;
}

static int bind_texture_cb(ClientData cd, Tcl_Interp* ip, int argc, char* argv[])
{
    if (argc != 3) {
        Tcl_AppendResult(ip, argv[0], ": invalid number of arguments\n", "usage: ", argv[0], "<object name> <texture name>", (char*)0);
        return TCL_ERROR;
    }
    if (!bind_texture(argv[1], argv[2])) {
        Tcl_AppendResult(ip, argv[0], ": could not bind texture ", argv[2], (char*)0);
        return TCL_ERROR;
    }
    return TCL_OK;
}
