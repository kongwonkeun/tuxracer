...
extern "C" void init_course_quadtree(scalar_t *elevation, int nx, int nz, scalar_t scalex, scalar_t scalez, point_t view_pos, scalar_t detail)
{
    HeightMapInfo hm;
    int i;
    ...
    // Debug info.
    print_debug(DEBUG_QUADTREE, (char*)"nodes = %d\n", root->CountNodes());
    print_debug(DEBUG_QUADTREE, (char*)"max error = %g\n", root->RecomputeError(root_corner_data));

    // Get rid of unnecessary nodes in flat-ish areas.
    print_debug(DEBUG_QUADTREE, (char*)"Culling unnecessary nodes (detail factor = %d)...\n", CULL_DETAIL_FACTOR);
