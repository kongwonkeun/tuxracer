...
scalar_t get_terrain_base_height(scalar_t distance)
{
    ...
    check_assertion(distance > -EPS, "distance should be positive");

    /* This will need to be fixed once we add variably-sloped terrain */
    base_height = -slope * distance - base_height_value / 255.0 * elev_scale;
    return base_height;
}
...
#undef byteval
    ...
    glColorPointer(4, GL_UNSIGNED_BYTE, STRIDE_GL_ARRAY, vnc_array + 8 * sizeof(GLfloat));
}
...
void load_course(char *course)
{
    ...
    if (_getcwd(cwd, BUFF_LEN) == NULL) { //---- kong ---- getcmd to _getcwd
        handle_system_error(1, "getcwd failed");
    }
    sprintf(buff, "%s/courses/%s", getparam_data_dir(), course);
    if (_chdir(buff) != 0) { //---- kong ---- chdir to _chdir
        handle_system_error(1, "Couldn't chdir to %s", buff);
    } 
    if (Tcl_EvalFile(g_game.tcl_interp, "./course.tcl") == TCL_ERROR) {
        handle_error(1, "Error evaluating %s/course.tcl: %s", buff, Tcl_GetStringResult(g_game.tcl_interp));
    } 
    if (_chdir(cwd) != 0) { //---- kong ---- chdir to _chdir
        handle_system_error(1, "Couldn't chdir to %s", cwd);
    }
    check_assertion(!Tcl_InterpDeleted(g_game.tcl_interp), "Tcl interpreter deleted");
    calc_normals();
    fill_gl_arrays();

    init_course_quadtree(elevation, nx, ny, course_width / (nx - 1.), -course_length / (ny - 1), g_game.player[local_player()].view.pos, getparam_course_detail_level());
    init_track_marks();
    course_loaded = True;
...
static int course_dim_cb(ClientData cd, Tcl_Interp *ip, int argc, char *argv[]) 
{
    double width, length;

    if ((argc != 3) && (argc != 5)) {
        Tcl_AppendResult(ip, argv[0], ": invalid number of arguments\n", "Usage: ", argv[0], " <course width> <course length>", " [<play width> <play length>]", (char*)0);
        return TCL_ERROR;
    } 
    if (Tcl_GetDouble(ip, argv[1], &width) != TCL_OK) {
        Tcl_AppendResult(ip, argv[0], ": invalid course width", (char*)0);
        return TCL_ERROR;
    } 
    if (Tcl_GetDouble(ip, argv[2], &length) != TCL_OK) {
        Tcl_AppendResult(ip, argv[0], ": invalid course length", (char*)0);
        return TCL_ERROR;
    } 
    course_width = width;
    ...
    if (argc == 5) {
        if (Tcl_GetDouble(ip, argv[3], &width) != TCL_OK) {
            Tcl_AppendResult(ip, argv[0], ": invalid play width", (char*)0);
            return TCL_ERROR;
        } 
        if (Tcl_GetDouble(ip, argv[4], &length) != TCL_OK) {
            Tcl_AppendResult(ip, argv[0], ": invalid play length", (char*)0);
            return TCL_ERROR;
        } 
        play_width = width;
        play_length = length;
...
static int angle_cb(ClientData cd, Tcl_Interp *ip, int argc, char *argv[]) 
{
    ...
    if (angle < MIN_ANGLE) {
        print_warning(TCL_WARNING, "course angle is too small. Setting to %f", MIN_ANGLE);
        angle = MIN_ANGLE;
    }
    if (MAX_ANGLE < angle) {
        print_warning(TCL_WARNING, "course angle is too large. Setting to %f", MAX_ANGLE);
        angle = MAX_ANGLE;
    }
    course_angle = angle;
    return TCL_OK;
...
static int elev_cb(ClientData cd, Tcl_Interp *ip, int argc, char *argv[]) 
{
    ...
    if (argc != 2) {
        Tcl_AppendResult(ip, argv[0], ": invalid number of arguments\n", "Usage: ", argv[0], " <elevation bitmap>", (char*)0);
        return TCL_ERROR;
    } 
    if (course_loaded) {
        print_warning(TCL_WARNING, "ignoring %s: course already loaded", argv[0]);
        return TCL_OK;
    }
    ...
    for (y = 0; y < ny; y++) {
        for (x = 0; x < nx; x++) {
            ELEV(nx - 1 - x, ny - 1 - y) = ((elev_img->data[(x + nx * y) * elev_img->sizeZ + pad] - base_height_value) / 255.0) * elev_scale - (scalar_t)(ny - 1. - y) / ny * course_length * slope;
        } 
...
static int terrain_cb(ClientData cd, Tcl_Interp *ip, int argc, char *argv[]) 
{
    ...
    if (argc != 2) {
        Tcl_AppendResult(ip, argv[0], ": invalid number of arguments\n", "Usage: ", argv[0], " <terrain bitmap>", (char*)0);
        return TCL_ERROR;
    } 
    ...
    if (terrain_img == NULL) {
        print_warning(TCL_WARNING, "%s: couldn't load %s", argv[0], argv[1]);
        Tcl_AppendResult(ip, argv[0], ": couldn't load ", argv[1], (char*)0);
        return TCL_ERROR;
    }
    if (nx != terrain_img->sizeX || ny != terrain_img->sizeY) {
        Tcl_AppendResult(ip, argv[0], ": terrain bitmap must have same dimensions as elevation bitmap", (char*)0);
        return TCL_ERROR;
    }
    ...
    for (y = 0; y < ny; y++) {
        for (x = 0; x < nx; x++) {
            ...
            terrain[idx] = intensity_to_terrain(terrain_img->data[(x + nx * y) * terrain_img->sizeZ+pad]);
        } 
...
{
    if (argc != 2) {
        Tcl_AppendResult(ip, argv[0], ": invalid number of arguments\n", "usage: ", argv[0], " <background image>", (char*)0);
        return TCL_ERROR;
    } 
...
{
    if (argc != 2) {
        Tcl_AppendResult(ip, argv[0], ": invalid number of arguments\n", "usage: ", argv[0], " <tree texture>", (char*)0);
        return TCL_ERROR;
    }
    if (tree_dep_call == -1) {
        if (num_tree_types + 1 >= MAX_TREE_TYPES) {
            Tcl_AppendResult(ip, argv[0], ": max number of tree types reached", (char*)0);
            return TCL_ERROR;
        }
    ...
    tree_types[tree_dep_call].texture = string_copy(tree_types[tree_dep_call].name);
    return TCL_OK;
}
...
{
    if (argc != 2) {
        Tcl_AppendResult(ip, argv[0], ": invalid number of arguments\n", "usage: ", argv[0], " <ice texture>", (char*)0);
        return TCL_ERROR;
    } 
...
{
    if (argc != 2) {
        Tcl_AppendResult(ip, argv[0], ": invalid number of arguments\n", "usage: ", argv[0], " <rock texture>", (char*)0);
        return TCL_ERROR;
    }
...
{
    if (argc != 2) {
        Tcl_AppendResult(ip, argv[0], ": invalid number of arguments\n", "usage: ", argv[0], " <snow texture>", (char*)0);
        return TCL_ERROR;
    } 
...
static int start_pt_cb(ClientData cd, Tcl_Interp *ip, int argc, char *argv[]) 
{
    ...
    if (argc != 3) {
        Tcl_AppendResult(ip, argv[0], ": invalid number of arguments\n", "Usage: ", argv[0], " <x coord> <y coord>", (char*)0);
        return TCL_ERROR;
    } 
    ...
    if (!(xcd > 0 && xcd < course_width)) {
        print_warning(TCL_WARNING, "%s: x coordinate out of bounds, using 0\n", argv[0]);
        xcd = 0;
    }
    if (!(ycd > 0 && ycd < course_length)) {
        print_warning(TCL_WARNING, "%s: y coordinate out of bounds, using 0\n", argv[0]);
        ycd = 0;
    }
    start_pt.x = xcd;
    start_pt.y = -ycd;
...
static int is_tree(unsigned char pixel[], tree_type_t ** which_type)
{
    ...
    *which_type = NULL;
    for (i = 0; i < num_tree_types; i++) {
        ...
        distance = abs(tree_types[i].red - pixel[0]) + abs(tree_types[i].green - pixel[1]) + abs(tree_types[i].blue - pixel[2]);
...
static int is_item(unsigned char pixel[], item_type_t** which_type)
{
    ...
    *which_type = NULL;
    for (i = 0; i < num_item_types; i++) {
        /* assume red green blue pixel ordering */
        distance = abs (item_types[i].red - pixel[0]) + abs(item_types[i].green - pixel[1]) + abs(item_types[i].blue - pixel[2]);
        if (distance < min_distance) {
            min_distance = distance;
...
static int trees_cb (ClientData cd, Tcl_Interp *ip, int argc, char *argv[]) 
{
    ...
    if (argc != 2) {
        Tcl_AppendResult(ip, argv[0], ": invalid number of arguments\n", "Usage: ", argv[0], " <tree location bitmap>", (char*)0);
        return TCL_ERROR;
    } 
    ...
    if (num_tree_types == 0 && num_item_types == 0) {
        print_warning(IMPORTANT_WARNING, "tux_trees callback called with no tree or item types set");
    }
    sx = treeImg->sizeX;
    sy = treeImg->sizeY;
    sz = treeImg->sizeZ;
    ...
    for (y = 0; y < sy; y++) {
        for (x = 0; x < sx; x++) {
            best_tree_dist = is_tree(&treeImg->data[(x + y * sx) * sz + pad], &which_tree);
            best_item_dist = is_item(&treeImg->data[(x + y * sx) * sz + pad], &which_item);
            ...
            if (best_tree_dist < best_item_dist && which_tree != NULL) {
                ...
                pos->x =  (sx - x) / (scalar_t)(sx - 1.) * course_width;
                pos->y = -(sy - y) / (scalar_t)(sy - 1.) * course_length;
                which_item->insert_pos = insert_list_elem(which_item->pos, which_item->insert_pos, (list_elem_data_t)pos);
            }
        }
        pad += (sx * sz) % 4; /* to compensate for word-aligned rows */
    }

    // double pass so that tree and object types are clumped together - reduce texture switching
    num_trees = 0;
    for (i = 0; i < num_tree_types; i++) {
        ...
        while (elem != NULL) {
            ...
            tree_locs[num_trees].height = (scalar_t)rand() / RAND_MAX * tree_types[i].vary * 2;
            tree_locs[num_trees].height -= tree_types[i].vary;
            tree_locs[num_trees].height = tree_types[i].height + tree_locs[num_trees].height * tree_types[i].height;
            tree_locs[num_trees].diam = (tree_locs[num_trees].height / tree_types[i].height) * tree_types[i].diam;
...
// deprecated
static int tree_size_cb(ClientData cd, Tcl_Interp *ip, int argc, char *argv[])
{
    double diam, height;

    if (argc != 3) {
        Tcl_AppendResult(ip, argv[0], ": invalid number of arguments\n", "Usage: ", argv[0], " <diameter> <height>", (char*)0);
        return TCL_ERROR;
    }
    if (Tcl_GetDouble(ip, argv[1], &diam) != TCL_OK) {
        Tcl_AppendResult(ip, argv[0], ": invalid diameter", (char*)0);
        return TCL_ERROR;
    } 
    if (Tcl_GetDouble(ip, argv[2], &height) != TCL_OK) {
        Tcl_AppendResult(ip, argv[0], ": invalid height", (char*)0);
        return TCL_ERROR;
    } 
    if (tree_dep_call == -1) {
        if (num_tree_types + 1 >= MAX_TREE_TYPES) {
...
/* The following functions are used to get the parameters for tux_tree_poly */

/* Gets the vertex list */
static int get_polyhedron_vertices(Tcl_Interp *ip, char *arg, int *num_vertices, point_t **vertex_list)
{
    int i;
    char** indices = 0;
    int rtn;
    ...
    if (rtn != TCL_OK)
    {
        Tcl_AppendResult(ip, "A vertex list must be provided\n", (char*)0);
...
static int get_single_polygon(Tcl_Interp *ip, char *arg, polygon_t *polygon)
{
    int i;
    char **indices=0;
    int rtn;
    ...
    if (rtn != TCL_OK)
    {
        Tcl_AppendResult(ip, "a list of vertices must be provided for each polygon\n", (char*)0);
        Tcl_Free((char*)indices);
        return (TCL_ERROR);
    }
...
static int get_polyhedron_polygon(Tcl_Interp* ip, char* arg, int* num_polygons, polygon_t** polygon_list)
{
    int i;
    char** indices = 0;
    int rtn;
    ...
    if(rtn != TCL_OK)
    {
        Tcl_AppendResult(ip, "A polygon list must be provided\n", (char*)0);
        Tcl_Free((char*)indices);
        return TCL_ERROR;
    }
...
// deprecated
static int tree_poly_cb(ClientData cd, Tcl_Interp *ip, int argc, char *argv[]) 
{
    int num_vertices;
    point_t* vertices;
    int num_polygons;
    ...
    if (argc != 3) {
        Tcl_AppendResult(ip, argv[0], "invalid number of arguments\nUsage: tux_tree_poly <vertex list> <face list> <polygon list>", (char*)0);
        return TCL_ERROR;
    }
    if (tree_dep_call == -1) {
        if (num_tree_types + 1 >= MAX_TREE_TYPES) {
    ...
    Tcl_AppendResult(ip, argv[0], ": error obtaining polyhedron data", (char*)0);
    return TCL_ERROR;
}

static int friction_cb (ClientData cd, Tcl_Interp *ip, int argc, char *argv[]) 
{
    double fric[3];
    scalar_t fric_s[3];
    int i;
    ...
    if (argc != 4) {
        ...
        Tcl_AppendResult(ip, argv[0], ": invalid number of arguments\n", "Usage: ", argv[0], " <ice coeff.> <rock coeff.> <snow coeff.>", (char*)0);
        return TCL_ERROR;
    }
    if (Tcl_GetDouble(ip, argv[1], &fric[0] ) != TCL_OK) {
        Tcl_AppendResult(ip, argv[0], ": invalid ice coefficient", (char*)0);
        return TCL_ERROR;
    }
    if (Tcl_GetDouble(ip, argv[2], &fric[1]) != TCL_OK) {
        Tcl_AppendResult(ip, argv[0], ": invalid rock coefficient", (char*)0);
        return TCL_ERROR;
    } 
    if (Tcl_GetDouble(ip, argv[3], &fric[2]) != TCL_OK) {
        Tcl_AppendResult(ip, argv[0], ": invalid snow coefficient", (char*)0);
        return TCL_ERROR;
    }
...
static int course_author_cb(ClientData cd, Tcl_Interp *ip, int argc, char *argv[]) 
{
    if (argc != 2) {
        Tcl_AppendResult(ip, argv[0], ": invalid number of arguments\n", "Usage: ", argv[0], " <author's name>", (char*)0);
        return TCL_ERROR;
    } 
    if (course_author != NULL) {
        free(course_author);
...
static int course_name_cb(ClientData cd, Tcl_Interp *ip, int argc, char *argv[]) 
{
    if (argc != 2) {
        Tcl_AppendResult(ip, argv[0], ": invalid number of arguments\n", "Usage: ", argv[0], " <course name>", (char*)0);
        return TCL_ERROR;
    } 
    if (course_name != NULL) {
        free(course_name);
...
static int base_height_value_cb(ClientData cd, Tcl_Interp *ip, int argc, char *argv[]) 
{
    int value;

    if (argc != 2) {
        Tcl_AppendResult(ip, argv[0], ": invalid number of arguments\n", "Usage: ", argv[0], " <base height>", (char*)0);
        return TCL_ERROR;
    } 
    if (Tcl_GetInt(ip, argv[1], &value) != TCL_OK) {
        Tcl_AppendResult(ip, argv[0], ": invalid base height", (char*)0);
        return TCL_ERROR;
    } 
...
static int tree_props_cb(ClientData cd, Tcl_Interp* ip, int argc, char *argv[])
{
    int i;
    int num_vertices;
    point_t* vertices;
    ...
    for (i = 1; (i < argc - 1) && !error; i += 2) {
        if (strcmp("-name", argv[i]) == 0) {
            ...
            if ( Tcl_GetDouble(ip, argv[i + 1], &tree_types[num_tree_types].diam) != TCL_OK) {
                Tcl_AppendResult(ip, argv[0], ": invalid diameter", (char*)0);
                error = 1;
            }
        } else if (strcmp("-height", argv[i]) == 0) {
            if ( Tcl_GetDouble(ip, argv[i + 1], &tree_types[num_tree_types].height) != TCL_OK) {
                Tcl_AppendResult(ip, argv[0], ": invalid height", (char*)0);
                error = 1;
            }
            ...
            if (!tree_types[num_tree_types].texture) {
                ...
                Tcl_AppendResult(ip, argv[0], ": specify only one texture", (char*)0);
            }
            ...
            if (num_col == 3 || num_col == 4) {
                ...
                Tcl_AppendResult(ip, argv[0], ": must specify three colours to link with tree type", (char*)0);
                error = 1;
            }
            Tcl_Free((char*)indices);
            ...
            if (rtn != TCL_OK || num_col != 2) {
                Tcl_AppendResult(ip, "two sublists of vertices and polygons must be specified\n", (char*)0);
                Tcl_Free((char*)indices);
                error = 1;
            }
            if (get_polyhedron_vertices(ip, indices[0], &num_vertices, &vertices) == TCL_OK) {
                if (get_polyhedron_polygon(ip, indices[1], &num_polys, &polys) == TCL_OK) {
            ...
            if (Tcl_GetDouble(ip, argv[i+1], &tree_types[num_tree_types].vary) != TCL_OK) {
                Tcl_AppendResult(ip, argv[0], ": invalid size variance", (char*)0);
                error = 1;
            }
        } else {
            print_warning(TCL_WARNING, "tux_props_cb: unrecognized parameter '%s'", argv[i]);
            /* not sure if next arg is valid command - try it */
            i -= 1;
        }
    }

    if (tree_types[num_tree_types].name == 0 || tree_types[num_tree_types].poly.vertices == 0 || tree_types[num_tree_types].texture == 0) {
        Tcl_AppendResult(ip, argv[0], ": some mandatory elements not filled", " tree name, texture name and tree polygon must be supplied.", (char*)0);
        ...
        free(tree_types[num_tree_types].poly.polygons);
        return TCL_ERROR;
    } else if (!bind_texture(tree_types[num_tree_types].name, tree_types[num_tree_types].texture)) {
        Tcl_AppendResult(ip, argv[0], ": could not bind texture ", tree_types[num_tree_types].texture, (char*)0);
        free(tree_types[num_tree_types].name);
        free(tree_types[num_tree_types].texture);
        free(tree_types[num_tree_types].poly.vertices);
        free(tree_types[num_tree_types].poly.polygons);
...
static int item_spec_cb(ClientData cd, Tcl_Interp *ip, int argc, char *argv[])
{
    int rtn, num_col;
    char** indices = NULL;
    int   convert_temp;
    ...
    if (num_item_types + 1 >= MAX_ITEM_TYPES) {
        Tcl_AppendResult(ip, argv[0], ": max number of item types reached", (char*)0);
        return TCL_ERROR;
    }
    ...
    while (*argv != NULL) {
        if (strcmp("-name", *argv) == 0) {
            ...
            if (Tcl_GetDouble(ip, *argv, &item_types[num_item_types].height) != TCL_OK) {
                Tcl_AppendResult(ip, argv[0], ": invalid height\n", (char*)0);
            }
        } else if (strcmp("-diameter", *argv) == 0) {
            NEXT_ARG;
            CHECK_ARG("-diameter", err_msg, item_spec_bail);
            if ( Tcl_GetDouble(ip, *argv, &item_types[num_item_types].diam) != TCL_OK) {
            ...
            if (Tcl_GetDouble(ip, *argv, &item_types[num_item_types].above_ground) != TCL_OK) {
            ...
            Tcl_Free((char*)indices);
            indices = NULL;
        } else if (strcmp("-nocollision", *argv) == 0 || strcmp("-nocollect", *argv) == 0) {
            ...
            if (get_tcl_tuple(ip, *argv, (scalar_t*)&(item_types[num_item_types].normal), 3) != TCL_OK) {
    ...
    if (item_types[num_item_types].name == 0 || (item_types[num_item_types].texture == 0 && item_types[num_item_types].reset_point == False)) {
        err_msg = "Some mandatory elements not filled. Item name and texture name must be supplied.";
        goto item_spec_bail;
    }
    if (item_types[num_item_types].reset_point == False && !bind_texture(item_types[num_item_types].name, item_types[num_item_types].texture)) {
...
item_spec_bail:
    ...
    Tcl_AppendResult(ip, "Error in call to tux_item_spec: ", err_msg, "\n", 
        "Usage: tux_item_spec -name <name> -height <height> -diameter <diameter> -colour {r g b [a]} [-texture <texture>] [-above_ground <distance>] [-nocollect] [-reset_point] [-normal {x y z}]", 
        (NULL) 
    );
    return TCL_ERROR;
}
