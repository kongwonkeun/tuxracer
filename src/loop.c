...
void register_loop_funcs(game_mode_t mode, mode_init_func_ptr_t init_func, mode_loop_func_ptr_t loop_func, mode_term_func_ptr_t term_func)
{
    check_assertion(mode >= 0 && mode < NUM_GAME_MODES, "invalid game mode");
...
/*
    ...
    it dispatches into the loop functions of the various game modes.
*/
void main_loop()
{
    ...
    if (g_game.mode != new_mode) {
        if (g_game.mode >= 0 && mode_funcs[g_game.mode].term_func != NULL) {
        ...
        if  (mode_funcs[g_game.mode].init_func != NULL) {
            ...
            /* reset time step clock so that there isn't a sudden jump when we start the new mode */
