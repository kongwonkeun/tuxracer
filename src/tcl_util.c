...
int get_tcl_tuple(Tcl_Interp *ip, char *inList, scalar_t *p, int n) 
{
    ...
    if ((TCL_OK != rtn) || (n != num_doubles)) {
        sprintf(s,"%d",n);
        Tcl_AppendResult(ip, "Expected a tuple of ", s, " doubles.\n", (char *) 0 );
...
int get_tcl_int_tuple(Tcl_Interp *ip, char *inList, int *p, int n) 
{
    ...
    if ((TCL_OK != rtn) || (n != num_ints)) {
        sprintf(s,"%d",n);
        Tcl_AppendResult(ip, "Expected a tuple of ", s, " integers.\n", (char *) 0 );
        Tcl_Free((char *)indices);
        return TCL_ERROR;
    }
...
{
#if defined(NATIVE_WIN32_COMPILER) /* only need to do this under Win32 */

    Tcl_Channel stdout_chnl, stderr_chnl;

    /* create new stdout channel */
    Tcl_SetStdChannel(NULL, TCL_STDOUT);
    stdout_chnl = Tcl_MakeFileChannel((ClientData)_get_osfhandle(_dup(_fileno(stdout))), TCL_WRITABLE);
    ...
    stderr_chnl = Tcl_MakeFileChannel((ClientData)_get_osfhandle(_dup(_fileno(stderr))), TCL_WRITABLE);
