...
static GLuint get_sphere_display_list( int divisions ) {
    ...
    check_assertion( idx >= 0 && idx < num_display_lists, "invalid number of sphere subdivisions" );
...
void traverse_dag( scene_node_t *node, material_t *mat )
{
    ...
    if ( node->geom == Sphere ) {
        ...
        if ( getparam_use_sphere_display_list() ) {
            glCallList( get_sphere_display_list( node->param.sphere.divisions ) );
        } else {
...
bool_t intersect_polygon( polygon_t p, point_t *v )
{
    ...
    for ( i=0; i < p.num_vertices; i++ ) {
        ...
        /* t is the distance from v0 of the closest point on the line to the origin */
...
bool_t check_polyhedron_collision_with_dag( scene_node_t *node, matrixgl_t modelMatrix, matrixgl_t invModelMatrix, polyhedron_t ph)
{
    ...
    while (child != NULL) {
        ...
        hit = check_polyhedron_collision_with_dag( child, newModelMatrix, newInvModelMatrix, ph );
