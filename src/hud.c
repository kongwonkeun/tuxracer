...
static void draw_time()
{
    ...
    if (!get_font_binding(binding, &font)) {
        print_warning( IMPORTANT_WARNING, "couldn't get font for binding %s", binding);
        return;
    }
    ...
    glPushMatrix();
    glTranslatef(TIME_LABEL_X_OFFSET, getparam_y_resolution() - TIME_LABEL_Y_OFFSET - asc, 0);
    ...
    get_font_metrics(font, string, &w, &asc, &desc);
    glPushMatrix();
    glTranslatef(TIME_X_OFFSET, time_y_refval - TIME_Y_OFFSET - asc, 0);
    draw_string(font, string);
    glPopMatrix();
    ...
    string = buff;
    glPushMatrix();
    glTranslatef(TIME_X_OFFSET + w + 5, time_y_refval - TIME_Y_OFFSET, 0);
    get_font_metrics(font, string, &w, &asc, &desc);
...
static void draw_herring_count( int herring_count )
{
    ...
    if ( !get_texture_binding( binding, &texobj ) ) {
        print_warning( IMPORTANT_WARNING, "Couldn't get texture for binding %s", binding );
        return;
    }
    ...
    glPushMatrix();
    {
        glTranslatef( getparam_x_resolution() - HERRING_ICON_X_OFFSET, getparam_y_resolution() - HERRING_ICON_Y_OFFSET - asc, 0 );

        glBegin( GL_QUADS );
        {
            glTexCoord2f( 0, 0 );
            glVertex2f( 0, 0 );
            glTexCoord2f( (GLfloat) HERRING_ICON_WIDTH / HERRING_ICON_IMG_SIZE, 0 );
            glVertex2f( HERRING_ICON_WIDTH, 0 );
            glTexCoord2f( (GLfloat)HERRING_ICON_WIDTH / HERRING_ICON_IMG_SIZE, (GLfloat)HERRING_ICON_HEIGHT / HERRING_ICON_IMG_SIZE );
            glVertex2f( HERRING_ICON_WIDTH, HERRING_ICON_HEIGHT );
            glTexCoord2f( 0, (GLfloat)HERRING_ICON_HEIGHT / HERRING_ICON_IMG_SIZE );
...
void draw_gauge( scalar_t speed, scalar_t energy )
{
    ...
    if ( !get_font_binding( binding, & speed_font ) ) {
        print_warning( IMPORTANT_WARNING, "Couldn't get font for binding %s", speed_font );
    }
    ...
    glPushMatrix();
    {
        glTranslatef( getparam_x_resolution() - GAUGE_WIDTH, 0, 0 );
        ...
        if ( speed > SPEEDBAR_GREEN_MAX_SPEED ) {
            ...
            if ( speed > SPEEDBAR_YELLOW_MAX_SPEED ) {
                ...
                if ( speed > SPEEDBAR_RED_MAX_SPEED ) {
                    ...
                    speedbar_frac += ( speed - SPEEDBAR_YELLOW_MAX_SPEED ) / ( SPEEDBAR_RED_MAX_SPEED - SPEEDBAR_YELLOW_MAX_SPEED ) * SPEEDBAR_RED_FRACTION;
                }
            } else {
                speedbar_frac += ( speed - SPEEDBAR_GREEN_MAX_SPEED ) / ( SPEEDBAR_YELLOW_MAX_SPEED - SPEEDBAR_GREEN_MAX_SPEED ) * SPEEDBAR_YELLOW_FRACTION;
            }
        } else {
        ...
        glPushMatrix();
        {
            glTranslatef( ENERGY_GAUGE_CENTER_X - w/2.0, ENERGY_GAUGE_BOTTOM + ENERGY_GAUGE_HEIGHT / 2.0, 0 );
        ...
        glPushMatrix();
        {
            glTranslatef( ENERGY_GAUGE_CENTER_X - w/2.0, ENERGY_GAUGE_BOTTOM + ENERGY_GAUGE_HEIGHT / 2.0 - asc - SPEED_UNITS_Y_OFFSET, 0 );
...
void print_fps()
{
    ...
    glPushMatrix();
    {
        glTranslatef( FPS_X_OFFSET, FPS_Y_OFFSET, 0 );
        draw_string( font, string );
    }
    glPopMatrix();
