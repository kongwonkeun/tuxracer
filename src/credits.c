...
static void draw_credits_text(scalar_t time_step)
{
    ...
    for (i = 0; i < sizeof(credit_lines) / sizeof(credit_lines[0]); i++) {
        ...
        if (!get_font_binding(line.binding, &font)) {
            print_warning(IMPORTANT_WARNING, "couldn't get font for binding %s", line.binding);
        } else {
    ...
    /* raw strips at the top and bottom to clip out text */
    ...
    glBegin(GL_QUADS);
    {
        ...
        glColor4f(ui_background_colour.r, ui_background_colour.g, ui_background_colour.b, 0);
...
{
    int status = 0;
    status |= add_keymap_entry(CREDITS, DEFAULT_CALLBACK, NULL, NULL, credits_key_cb);
