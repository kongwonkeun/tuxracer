...
typedef struct {
    ...
    save_data_t data;
    char future_use[STANDARD_RECORD_SIZE - sizeof(save_data_t) -  sizeof(save_data_type_t)];
}
save_info_t;
...
typedef struct {
    ...
    int score;
    char future_use[STANDARD_RECORD_SIZE - sizeof(single_event_t) - sizeof(int) - MAX_PLAYER_NAME_LENGTH];
}
score_info_t;
...
/*
 * score_table is indexed on event and references pointers to hash tables
 * that are indexed on cup and reference score_info_t* event->cup->score
 */
...
int get_high_score_file_name(char *buff, int len)
{
    ...
    if (strlen(buff) + strlen(HIGH_SCORE_FILE) + 2 > (size_t)len) { //---- kong ---- add size_t
        return 1;
    }
#if defined(WIN32)
    strcat(buff, "\\");
...
int get_save_game_file_name(char *buff, char *player, int len)
{
    ...
    if (strlen(buff) + strlen(player) + 6 > (size_t)len) { //---- kong ---- add size_t
        return 1;
    }
    ...
    if (get_high_score_file_name(score_file, sizeof(score_file)) == 0) {
        ...
        if (score_stream != NULL) {
            while (fread(&this_score, sizeof(this_score), 1, score_stream)) {
                set_high_score(this_score.event.event, this_score.event.cup, this_score.player_name, this_score.score);
            }
...
bool_t set_high_score(char* event, char* cup, char* player, int score)
{
    ...
    if (!get_hash_entry(score_table, event, (hash_entry_t*)&cup_table)) {
        ...
        if (cup_table == NULL) return False; //---- kong ---- add this line
    ...
    if (!get_hash_entry(*cup_table, cup, (hash_entry_t*)&this_score)) {
        ...
        if (this_score == NULL) return False; //---- kong ---- add this line
...
{
    int sav_index;
    sav_index = (int)strlen(str) - 1; //---- kong ---- add (int)
    if (str[sav_index] != 'v') {
        return False;
    }
    sav_index--;
...
void init_saved_games(void) 
{
    ...
    if (dir_file_list == NULL) {
        /* config dir doesn't exist. this is a normal condition for the first time the program is run */
        return;
    }

    for (cur_dir_file = get_list_head(dir_file_list); cur_dir_file != NULL; cur_dir_file = get_next_list_elem(dir_file_list, cur_dir_file))
    {
        ...
        if (get_sav_index(cur_dir_filename, &sav_index)) {
            ...
            sprintf(file_name, "%s" DIR_SEPARATOR "%s", dir_name, cur_dir_filename);

            save_stream = fopen(file_name, "r");

            if (fread(magic, sizeof(magic), 1, save_stream) != 1 || strncmp(magic, SAVE_MAGIC_V1, sizeof(magic)) != 0) //---- kong ---- ????
            {
                print_warning(IMPORTANT_WARNING, "`%s' is not a valid saved game file", file_name);
                fclose(save_stream);
                continue;
            }
            ...
            if (save_stream != NULL) {
                while (fread(&this_save, sizeof(this_save), 1, save_stream)) {
                    ...
                    case EVENT_INFO:
                        set_last_completed_cup(player_name, this_save.data.event.event, this_save.data.event.difficulty, this_save.data.event.cup);
                        print_debug(DEBUG_SAVE, "Read completed from `%s': name: %s, event: %s, difficulty: %d, cup: %s",
                            cur_dir_filename,
                            player_name,
                    ...
                    case RACE_RESULTS:
                        ...
                        print_debug(DEBUG_SAVE, "Read results from `%s': name: %s, event: %s, cup: %s, race: %s, difficulty: %d, time: %g, herring: %d, score: %d",
                            cur_dir_filename,
                            player_name,
                            this_save.data.results.event,
                            this_save.data.results.cup,
                    ...
                    default:
                        print_warning(IMPORTANT_WARNING, "Unrecognized data type in save file.");
                    }
                }
                ...
                print_warning(IMPORTANT_WARNING, "Couldn't read file `%s': %s", file_name, strerror(errno));
            }
        }
    }
...
bool_t get_last_completed_cup(char* player, char* event, difficulty_level_t d, char** cup)
{
...
bool_t set_last_completed_cup(char* player, char* event, difficulty_level_t d, char* cup)
{
    hash_table_t event_table;
    save_info_t *this_save;
    ...
    if (!get_hash_entry(event_table, event, (hash_entry_t*)&this_save)) {
        this_save = (save_info_t*)malloc(sizeof(save_info_t) * DIFFICULTY_NUM_LEVELS);
        if (this_save == NULL) return False; //---- kong ---- add this line
...
bool_t get_saved_race_results(char *player, char *event, char *cup, char *race, difficulty_level_t d, scalar_t *time, int *herring, int *score)
{
...
bool_t set_saved_race_results(char *player, char *event, char *cup, char *race, difficulty_level_t d, scalar_t time, int herring, int score)
{
    hash_table_t player_table;
    hash_table_t event_table;
    hash_table_t cup_table;
    ...
    check_assertion(this_save->data_type == RACE_RESULTS, "Invalid data type");
...
static void truncate_and_init_save_file(char *player_name)
{
    ...
    if (get_save_game_file_name(save_file, player_name, sizeof(save_file)) == 0) 
    {
        save_stream = fopen(save_file, "w");
        if (save_stream == NULL) {
            print_warning(IMPORTANT_WARNING, "couldn't open `%s' for writing: %s", save_file, strerror(errno));
        } else {
            check_assertion(strlen(SAVE_MAGIC_V1) == 4, "Magic number has wrong size");
            fwrite(SAVE_MAGIC_V1, 4, 1, save_stream);

            if (fclose(save_stream) != 0) {
                print_warning(IMPORTANT_WARNING, "couldn't close `%s': %s", save_file, strerror(errno));
            }
        }
    } else {
        print_warning(IMPORTANT_WARNING, "couldn't get save game file for player `%s'",  player_name);
    }
}
...
void write_saved_games(void)
{
    ...
    begin_hash_scan(progress_save_table, &player_scan_ptr);
    while (next_hash_entry(player_scan_ptr, &player_name, (hash_entry_t*)&event_table)) 
    {
        if (get_save_game_file_name(save_file, player_name, sizeof(save_file)) == 0) 
        {
            ...
            if (save_stream == NULL) {
                ...
                while (next_hash_entry(save_scan_ptr, &event_name, (hash_entry_t*)&this_save)) 
                {
                    for(i = 0; i < DIFFICULTY_NUM_LEVELS; i++ ) {
                        level = (difficulty_level_t)i;
                        if (this_save[level].data_type >= 0 ) {
                            fwrite(&(this_save[level]), sizeof(this_save[level]), 1, save_stream);
                        }
                    }
                }
        ...
        while (next_hash_entry(player_scan_ptr, &player_name, (hash_entry_t*)&event_table)) 
        {
            if (get_save_game_file_name(save_file, player_name, sizeof(save_file)) == 0)
            {
                ...
                if (save_stream == NULL) {
                    ...
                    while (next_hash_entry(event_scan_ptr, NULL, (hash_entry_t*)&cup_table)) 
                    {
                        begin_hash_scan(cup_table, &cup_scan_ptr);
                        while (next_hash_entry(cup_scan_ptr, NULL, (hash_entry_t*)&race_table))
                        {
                            begin_hash_scan(race_table, &race_scan_ptr);
                            while (next_hash_entry(race_scan_ptr, NULL, (hash_entry_t*)&this_save))
                            {
                                fwrite(this_save, sizeof(save_info_t), 1, save_stream);
                            }
