...
static bool_t is_current_race_completed(void)
{
    ...
    if (compare_race_positions(cup_data, cur_elem, last_completed_race) >= 0) {
        return True;
    }
    return False;
}
...
static bool_t is_current_race_first_incomplete(void)
{
    ...
    if (compare_race_positions(cup_data, last_completed_race, cur_elem) == 1) {
        return True;
    }
    return False;
}
...
void update_race_data(void)
{
    ...
    if (g_game.practicing) {
        ...
        if (cup_complete && mirror_ssbtn != NULL && conditions_ssbtn != NULL && wind_ssbtn != NULL && snow_ssbtn != NULL) {
...
static void update_race_results(void)
{
    ...
    if (!get_saved_race_results(plyr->name, event, cup, race_name, g_game.difficulty, &time, &herring, &score))
    {
        update_score = True;
    ...
    if (update_score) {
        bool_t result;
        result =  set_saved_race_results(plyr->name, event, cup, race_name, g_game.difficulty, g_game.time, plyr->herring, plyr->score); 
        if (!result) {
            print_warning(IMPORTANT_WARNING, "couldn't save race results");
        }
    }
}
...
void update_for_won_race(void)
{
    ...
    check_assertion(g_game.practicing == False, "Tried to update for won race in practice mode");
    race_data = (race_data_t*)get_list_elem_data(cur_elem);
    if (last_completed_race == NULL || compare_race_positions(cup_data, last_completed_race, cur_elem) > 0)
    {
        last_completed_race = cur_elem;
        if (cur_elem == get_list_tail(race_list)) {
            cup_complete = True;
            if (!set_last_completed_cup(plyr->name, g_game.current_event, g_game.difficulty, g_game.current_cup))
            {
                print_warning(IMPORTANT_WARNING, "couldn't save cup completion");
            } else {
                print_debug(DEBUG_GAME_LOGIC, "Cup %s completed", g_game.current_cup);
            }
        }
    }
...
static void race_listbox_item_change_cb(listbox_t *listbox, void *userdata)
{
    ...
    if (g_game.practicing) {
        ...
        if (cup_complete && conditions_ssbtn && wind_ssbtn && snow_ssbtn && mirror_ssbtn) 
        {
            ssbutton_set_state(conditions_ssbtn, (int)data->conditions);
            ssbutton_set_state(wind_ssbtn, (int)data->windy);
            ssbutton_set_state(snow_ssbtn, (int)data->snowing);
            ssbutton_set_state(mirror_ssbtn, (int)data->mirrored);
        }
        update_button_enabled_states();
        ui_set_dirty();
    } 
...
void draw_status_msg(int x_org, int y_org, int box_width, int box_height)
{
    ...
    if (!g_game.practicing) {
        if (is_current_race_completed()) {
            ...
            if (!get_saved_race_results(plyr->name, g_game.current_event, g_game.current_cup, race_data->name, g_game.difficulty, &time, &herring, &score))
            {
                difficulty_level_t d = g_game.difficulty;
                print_warning(IMPORTANT_WARNING, "No saved results for race `%s'.  Using race minimum requirements.", race_data->name);
        ...
        data = (open_course_data_t*)get_list_elem_data(cur_elem);
        msg = "Best result:";
        if (!get_saved_race_results(plyr->name, g_game.current_event, g_game.current_cup, data->name, g_game.difficulty, &time, &herring, &score))
        {
    ...
    if (!get_font_binding("race_requirements", &font) || !get_font_binding("race_requirements_label", &label_font))
    {
        print_warning(IMPORTANT_WARNING, "couldn't get fonts for race requirements");
    } else {
        glPushMatrix();
        {
            glTranslatef(x_org + 0, y_org + 200, 0);
        ...
        if (draw_stats) {
            glPushMatrix();
            {
                ...
                glTranslatef(x_org + 0, y_org + 184, 0);
                bind_font_texture(label_font);
...
static void set_widget_positions_and_draw_decorations()
{
    ...
    button_set_position(back_btn, make_point2d(x_org + 131  - button_get_width(back_btn)  / 2.0, 42));
    button_set_position(start_btn, make_point2d(x_org + 343 - button_get_width(start_btn) / 2.0, 42));
    listbox_set_position(race_listbox, make_point2d(x_org, y_org + 221));
    textarea_set_position(desc_ta, make_point2d(x_org, y_org + 66));
    
    if (g_game.practicing || (cup_complete && conditions_ssbtn && wind_ssbtn && snow_ssbtn && mirror_ssbtn)) 
    {
        ssbutton_set_position(conditions_ssbtn, make_point2d(x_org + box_width - 4 * 36 + 4, y_org + 181));
        ssbutton_set_position(wind_ssbtn, make_point2d(x_org + box_width - 3 * 36 + 4,       y_org + 181));
        ssbutton_set_position(snow_ssbtn, make_point2d(x_org + box_width - 2 * 36 + 4,       y_org + 181));
        ssbutton_set_position(mirror_ssbtn, make_point2d(x_org + box_width - 1 * 36 + 4,     y_org + 181));
        ...
        glPushMatrix();
        {
            glTranslatef(x_org + box_width - 4 * 36 + 4, y_org + 181, 0);
            check_assertion(INIT_NUM_LIVES == 4, "Assumption about number of lives invalid -- need to recode this part");
            ...
            for (i = 0; i < 4; i++) {
                ...
                glBegin( GL_QUADS );
                {
                    glTexCoord2f( ll.x, ll.y ); glVertex2f( 0, 0 );
                    glTexCoord2f( ur.x, ll.y ); glVertex2f( 32, 0 );
                    glTexCoord2f( ur.x, ur.y ); glVertex2f( 32, 32 );
                    glTexCoord2f( ll.x, ur.y ); glVertex2f( 0, 32 );
                }
                glEnd();
                glTranslatef( 36, 0, 0 );
            }
    ...
    if (!get_font_binding("menu_label", &font)) {
        print_warning(IMPORTANT_WARNING, "couldn't get font for binding menu_label");
    } else {
        bind_font_texture(font);
        ...
        glPushMatrix();
        {
            glTranslatef(x_org + box_width / 2.0 - text_width / 2.0, y_org + 310 - asc, 0);
            draw_string(font, string);
        }
        glPopMatrix();
    }
    /* Draw text indicating race requirements (if race not completed), or results in race if race completed. */
    ...
    glBegin( GL_QUADS );
    {
        glTexCoord2d( 0, 0); glVertex2f( x_org+box_width-136, y_org+70 );
        glTexCoord2d( 1, 0); glVertex2f( x_org+box_width-4, y_org+70 );
        glTexCoord2d( 1, 1); glVertex2f( x_org+box_width-4, y_org+70+99 );
        glTexCoord2d( 0, 1); glVertex2f( x_org+box_width-136, y_org+70+99 );
    }
    glEnd();
}
...
static void race_select_init(void)
{
    ...
    if (g_game.practicing) {
        ...
        event_data = (event_data_t*)get_list_elem_data( get_event_by_name(g_game.current_event));
        check_assertion(event_data != NULL, "couldn't find current event.");
        cup_data = (cup_data_t*)get_list_elem_data( get_event_cup_by_name(event_data, g_game.current_cup));
        check_assertion(cup_data != NULL, "¤ºouldn't find current cup." );
        race_list = get_cup_race_list(cup_data);
        conv_func = get_name_from_race_data;
    }
    /* Unless we're coming back from a race, initialize the race data to defaults. */
    if (g_game.prev_mode != GAME_OVER) {
        ...
        if (g_game.practicing) {
            ...
            if (is_cup_complete(event_data, get_event_cup_by_name(event_data, g_game.current_cup)))
            {
                cup_complete = True;
        ...
        if (!g_game.practicing && !cup_complete) {
            ...
            print_debug(DEBUG_GAME_LOGIC, "¤ºurrent lives: %d", plyr->lives);
        }
    }
    back_btn = button_create(dummy_pos, 150, 40, "button_label", "Back");
    ...
    start_btn = button_create(dummy_pos, 150, 40, "button_label", "Race!");
    ...
    race_listbox = listbox_create(dummy_pos, 460, 44, "listbox_item", race_list, conv_func);
    listbox_set_current_item(race_listbox, cur_elem);
    listbox_set_item_change_event_cb(race_listbox, race_listbox_item_change_cb, NULL);
    listbox_set_visible(race_listbox, True);

    /* Create text area */
    desc_ta = textarea_create(dummy_pos, 312, 107, "race_description", "");
    if (g_game.practicing) {
        open_course_data_t *data;
        data = (open_course_data_t*)get_list_elem_data(cur_elem);
        textarea_set_text(desc_ta, data->description);
    ...
    if (g_game.practicing || cup_complete) {
        /* mirror */
        mirror_ssbtn = ssbutton_create(dummy_pos, 32, 32, 2);
        ssbutton_set_state_image(mirror_ssbtn, 0, "mirror_button", make_point2d(0.0  / 64.0, 32.0 / 64.0), make_point2d(32.0 / 64.0, 64.0 / 64.0), white);
        ssbutton_set_state_image(mirror_ssbtn, 1, "mirror_button", make_point2d(32.0 / 64.0, 32.0 / 64.0), make_point2d(64.0 / 64.0, 64.0 / 64.0), white);
        ssbutton_set_state(mirror_ssbtn, (int)g_game.race.mirrored );
        ssbutton_set_visible(mirror_ssbtn, True);
        /* conditions */
        conditions_ssbtn = ssbutton_create(dummy_pos, 32, 32, 3);
        ssbutton_set_state_image(conditions_ssbtn, 0, "conditions_button", make_point2d(0.0  / 64.0, 32.0 / 64.0), make_point2d(32.0 / 64.0, 64.0 / 64.0), white);
        ssbutton_set_state_image(conditions_ssbtn, 1, "conditions_button", make_point2d(32.0 / 64.0, 0.0 / 64.0), make_point2d(64.0 / 64.0, 32.0 / 64.0), white);
        ssbutton_set_state_image(conditions_ssbtn, 2, "conditions_button", make_point2d(32.0 / 64.0, 32.0 / 64.0), make_point2d(64.0 / 64.0, 64.0 / 64.0), white);
        ssbutton_set_state(conditions_ssbtn, (int)g_game.race.conditions );
        ssbutton_set_visible(conditions_ssbtn, True);
        /* wind */
        wind_ssbtn = ssbutton_create(dummy_pos, 32, 32, 2);
        ssbutton_set_state_image(wind_ssbtn, 0, "wind_button", make_point2d(0.0  / 64.0, 32.0 / 64.0), make_point2d(32.0 / 64.0, 64.0 / 64.0), white);
        ssbutton_set_state_image(wind_ssbtn, 1, "wind_button", make_point2d(32.0 / 64.0, 32.0 / 64.0), make_point2d(64.0 / 64.0, 64.0 / 64.0), white);
        ssbutton_set_state(wind_ssbtn, (int)g_game.race.windy);
        ssbutton_set_visible(wind_ssbtn, True);
        /* snow */
        snow_ssbtn = ssbutton_create(dummy_pos, 32, 32, 2);
        ssbutton_set_state_image(snow_ssbtn, 0, "snow_button", make_point2d(0.0  / 64.0, 32.0 / 64.0), make_point2d(32.0 / 64.0, 64.0 / 64.0), white);
        ssbutton_set_state_image(snow_ssbtn, 1, "snow_button", make_point2d(32.0 / 64.0, 32.0 / 64.0), make_point2d(64.0 / 64.0, 64.0 / 64.0), white);
...
static void race_select_loop(scalar_t time_step)
{
    ...
    if (getparam_ui_snow()) {
        update_ui_snow(time_step, (bool_t)(wind_ssbtn != NULL && ssbutton_get_state(wind_ssbtn)));
        draw_ui_snow();
    }
...
START_KEYBOARD_CB(race_select_key_cb)
{
    ...
    if (special) {
        ...
        case 27: /* Esc */
            ...
            break;
        case 'c': next_race_condition(); break;
        case 'w': toggle_wind(); break;
        case 'm': toggle_mirror(); break;
        case 's': break;
        }
    }
    ui_check_dirty();
...
{
    int status = 0;
    status |= add_keymap_entry(RACE_SELECT, DEFAULT_CALLBACK, NULL, NULL, race_select_key_cb);
    check_assertion(status == 0, "out of keymap entries");
    register_loop_funcs(RACE_SELECT, race_select_init, race_select_loop, race_select_term);
}
