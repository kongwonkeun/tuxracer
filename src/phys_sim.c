...
#define MAX_TURN_PENALTY        0.15    // Maximum increase in friction when turning
#define BRAKE_FORCE             200     // Force applied by Tux when braking (N)
#define MAX_ROLL_ANGLE          30      // Maximum roll angle (degrees)
#define BRAKING_ROLL_ANGLE      55      // Roll angle while braking (degrees)
#define IDEAL_ROLL_FRIC_COEFF   0.35    // Baseline friction coeff for rolling
#define IDEAL_ROLL_SPEED        6.0     // Baseline speed for rolling (m/s)
#define TUX_ORIENTATION_TIME_CONSTANT           0.14    // Tux's orientation is updated using a first-order filter; when tux is on ground (s)
#define TUX_ORIENTATION_AIRBORNE_TIME_CONSTANT  0.5     // Tux's orientation is updated using a first-order filter; when tux is airborne (s)
#define MAX_TURN_PARTICLES      1500    // Max particles generated by turning (particles/s)
#define MAX_ROLL_PARTICLES      3000    // Max particles generated by rolling (particles/s)
#define BRAKE_PARTICLES         4000    // Particles generated by braking (particles/s)
#define PARTICLE_SPEED_FACTOR   40      // Number of particles is multiplied by (speed / PARTICLES_SPEED_FACTOR)
#define MAX_PARTICLE_ANGLE      80      // Maximum deflection angle for particles (out to side of player)
#define MAX_PARTICLE_ANGLE_SPEED    50  // Speed at which maximum deflection occurs
#define PARTICLE_SPEED_MULTIPLIER   0.3 // Particle speed = min(MAX_PARTICLE_SPEED, speed*PARTICLE_SPEED_MULTIPLIER)
#define MAX_PARTICLE_SPEED          2
#define TUX_GLUTE_STAGE_1_COMPRESSIBILITY   0.05    // The compressiblity of the first section of the spring modelling Tux's tush (m)
#define TUX_GLUTE_STAGE_1_SPRING_COEFF      1500    // The spring coefficient of first section of aforementioned tush (N/m)
#define TUX_GLUTE_STAGE_1_DAMPING_COEFF     100     // The damping coefficient of first section of aforementioned tush (N*s/m)
#define TUX_GLUTE_STAGE_2_COMPRESSIBILITY   0.12    // The compressiblity of the second section of the spring modelling Tux's tush (m)
#define TUX_GLUTE_STAGE_2_SPRING_COEFF      3000    // The spring coefficient of second section of aforementioned tush (N/m)
#define TUX_GLUTE_STAGE_2_DAMPING_COEFF     500     // The damping coefficient of second section of aforementioned tush (N*s/m)
#define TUX_GLUTE_STAGE_3_SPRING_COEFF      10000   // The spring coefficient of third section of aforementioned tush (N/m)
#define TUX_GLUTE_STAGE_3_DAMPING_COEFF     1000    // The damping coefficient of third section of aforementioned tush (N*s/m)
#define TUX_GLUTE_MAX_SPRING_FORCE          3000    // The maximum force exertedd by both sections of spring (N)
#define MAX_SURFACE_PENETRATION             0.2     // Maximum distance that player can penetrate into terrain (m)
#define AIR_DENSITY     1.308       // Density of air @ -3 C (kg/m^3)
#define TUX_DIAMETER    TUX_WIDTH   // Diameter of sphere used to model Tux for air resistance calcs (m)
#define AIR_VISCOSITY   17.00e-6    // Viscosity of air @ -3 C (N*s/m^2)

/* Drag coefficient vs. Reynolds number table (from Janna, _Introduction to Fluid Mechanics_, 3rd Edition, PWS-Kent, p 308) 
   Values are for Re in the range of 10^-1 to 10^6, spaced logarithmically (all values are log base 10) */
...
#define MIN_TIME_STEP       0.01    // Minimum time step for ODE solver (s)
#define MAX_TIME_STEP       0.10    // Maximum time step for ODE solver (s)
#define MAX_STEP_DISTANCE   0.20    // Maximum distance of step for ODE solver (m)
#define MAX_POSITION_ERROR  0.005   // Tolerance on error in Tux's position (m)
#define MAX_VELOCITY_ERROR  0.05    // Tolerance on error in Tux's velocity (m/s)

/* To smooth out the terrain, the terrain normals are interpolated near the edges of the triangles.
   This parameter controls how much smoothing is done, as well as the size of the triangle boundary over which smoothing occurs.
   A value of 0 will result in no smoothing, while a value of 0.5 will smooth the normals over 
   the entire surface of the triangle (though the smoothing will increase
   as the value approaches infinity). (dimensionless) */
#define NORMAL_INTERPOLATION_LIMIT  0.05
#define TUX_Y_CORRECTION_ON_STOMACH 0.33    // The distance to move Tux up by so that he doesn't sit too low on the ground (m)
#define COLLISION_TOLERANCE         0.04    // The square of the distance that Tux must move before we recompute a collision (m^2)
#define PADDLING_DURATION           0.40    // Duraction of paddling motion (s)
#define MAX_PADDLING_FORCE          122.5   // Force applied against ground when paddling (N)
#define IDEAL_PADDLING_FRIC_COEFF   0.35    // Ideal paddling friction coefficient
#define BASE_JUMP_G_FORCE           1.5     // G force applied for jump with charge of 0
#define MAX_JUMP_G_FORCE            3       // Maximum G force applied during jump
#define DAMAGE_RESISTANCE       (4.0 * TUX_MASS * EARTH_GRAV)   // Magnitude of force before damage is incurred (N)
#define DAMAGE_SCALE            9e-8    // Damage scaling factor (health/(N*s))
#define COLLISION_BASE_DAMAGE   0.02    // Amount to scale tree force by (so that it does more damage than regular collisions) (dimensionless)
#define PADDLING_DAMAGE         0.02    // Damage incurred by paddling exertion (health/s)
...
void get_indices_for_point(scalar_t x, scalar_t z, int *x0, int *y0, int *x1, int *y1)
{
    scalar_t courseWidth, courseLength;
    scalar_t xidx, yidx;
    int nx, ny;
...
void find_barycentric_coords(scalar_t x, scalar_t z, index2d_t *idx0, index2d_t *idx1, index2d_t *idx2, scalar_t *u, scalar_t *v)
{
    scalar_t xidx, yidx;
    int nx, ny;
    int x0, x1, y0, y1;
...
void set_tux_pos(player_data_t *plyr, point_t new_pos)
{
    ...
    translate_scene_node(tuxRoot, make_vector(new_pos.x, disp_y, new_pos.z));
} 

bool_t check_tree_collisions(player_data_t *plyr, point_t pos, point_t *tree_loc, scalar_t *tree_diam)
{
    tree_t *trees;
    int num_trees, i;
    scalar_t diam = 0.0; 
    ...
    for (i = 0; i < num_trees; i++) {
        ...
        /* check distance from tree; .6 is the radius of a bounding sphere around tux (approximate) */
        ...
        translate_scene_node(tux_root, make_vector(pos.x, pos.y, pos.z));
...
scalar_t get_compression_depth(terrain_t surf_type) 
{
    ...
    case Rock:  return 0.01;
    case Snow:  return 0.11;
    default:    code_not_reached();
    }
    return 0;
}

/* Check for tree collisions and adjust position and velocity appropriately. */
static void adjust_for_tree_collision(player_data_t *plyr, point_t pos, vector_t *vel)
{
    vector_t treeNml;
    point_t treeLoc;
    bool_t treeHit;
    ...
    if (treeHit) {
        /* Calculate the normal vector to the tree; here we model the tree as a vertical cylinder. */
        ...
        /* If Tux is moving into the tree, reduce the speed further, and reflect the velocity vector off of the tree */
        ...
        if (costheta < 0) {
            ...
            speed *= 1 + costheta; 
            /* Do the reflection */
            *vel = add_vectors(scale_vector(-2. * dot_product(*vel, treeNml), treeNml), *vel);
...
/* Adjusts velocity so that his speed doesn't drop below the minimum speed */
scalar_t adjust_velocity(vector_t *vel, point_t pos, plane_t surf_plane, scalar_t dist_from_surface)
{
    vector_t surf_nml;
    scalar_t speed;
    ...
    if (speed < EPS) {
        if (fabs(surf_nml.x) + fabs(surf_nml.z) > EPS) {
            *vel = project_into_plane(surf_nml, make_vector(0.0, -1.0, 0.0));
            normalize_vector(vel);
    } else {
...
void adjust_position(point_t *pos, plane_t surf_plane, scalar_t dist_from_surface)
{
    if (dist_from_surface < -MAX_SURFACE_PENETRATION) {
        *pos = move_point(*pos, scale_vector(-MAX_SURFACE_PENETRATION - dist_from_surface, surf_plane.nml));
    }
    return;
}
...
static vector_t adjust_tux_zvec_for_roll(player_data_t *plyr, vector_t vel, vector_t zvec)
{
    ...
    if (plyr->control.is_braking) {
        make_rotation_about_vector_matrix(rot_mat, vel, plyr->control.turn_fact * BRAKING_ROLL_ANGLE);
    } else {
        make_rotation_about_vector_matrix(rot_mat, vel, plyr->control.turn_fact * MAX_ROLL_ANGLE);
    }
    return transform_vector(rot_mat, zvec);
}

static vector_t adjust_surf_nml_for_roll(player_data_t *plyr, vector_t vel, scalar_t fric_coeff, vector_t nml)
{
    matrixgl_t rot_mat; 
    scalar_t angle;
    scalar_t speed;
...
void adjust_orientation(player_data_t *plyr, scalar_t dtime, point_t pos, vector_t vel, scalar_t dist_from_surface, vector_t surf_nml)
{
    ...
    plyr->orientation = interpolate_quaternions(plyr->orientation, new_orient, min(dtime / time_constant, 1.0));
    ...
    make_rotation_about_vector_matrix(rot_mat, new_y, (plyr->control.barrel_roll_factor * 360));
    multiply_matrices(cob_mat, rot_mat, cob_mat);
    new_x = make_vector(cob_mat[0][0], cob_mat[0][1], cob_mat[0][2]); 
    make_rotation_about_vector_matrix(rot_mat, new_x, plyr->control.flip_factor * 360);
    multiply_matrices(cob_mat, rot_mat, cob_mat);
...
void generate_particles(player_data_t *plyr, scalar_t dtime, point_t pos, scalar_t speed) 
{
    ...
    if (surf_weights[Snow] > 0.5 && pos.y < surf_y) {
        ...
        right_part_pt = move_point(right_part_pt, scale_vector(TUX_WIDTH / 2.0, xvec));
        left_part_pt  = move_point(left_part_pt, scale_vector(-TUX_WIDTH / 2.0, xvec));
        right_part_pt.y = left_part_pt.y  = surf_y;
        brake_particles = dtime * BRAKE_PARTICLES * (plyr->control.is_braking ? 1.0 : 0.0) * min(speed / PARTICLE_SPEED_FACTOR, 1.0);
        ...
        left_particles  = turn_particles * fabs(min(plyr->control.turn_fact, 0.)) + brake_particles + roll_particles * fabs(min(plyr->control.turn_animation, 0.));
        right_particles = turn_particles * fabs(max(plyr->control.turn_fact, 0.)) + brake_particles + roll_particles * fabs(max(plyr->control.turn_animation, 0.));
        ...
        make_rotation_about_vector_matrix(rot_mat, plyr->direction, max(-MAX_PARTICLE_ANGLE, -MAX_PARTICLE_ANGLE * speed / MAX_PARTICLE_ANGLE_SPEED));
        left_part_vel = transform_vector(rot_mat, plyr->plane_nml);
        left_part_vel = scale_vector(min(MAX_PARTICLE_SPEED, speed * PARTICLE_SPEED_MULTIPLIER), left_part_vel);

        make_rotation_about_vector_matrix(rot_mat, plyr->direction, min( MAX_PARTICLE_ANGLE,  MAX_PARTICLE_ANGLE * speed / MAX_PARTICLE_ANGLE_SPEED));
        right_part_vel = transform_vector(rot_mat, plyr->plane_nml);
        right_part_vel = scale_vector(min(MAX_PARTICLE_SPEED, speed * PARTICLE_SPEED_MULTIPLIER), right_part_vel);
...
vector_t calc_wind_force(vector_t player_vel)
{
    ...
    if (g_game.race.windy) {
        ...
        if (last_time_called != g_game.time) {
            wind_scale = wind_scale + (rand() / (scalar_t)RAND_MAX - 0.50) * 0.15;
            wind_scale = min(1.0, max(0.0, wind_scale));
        }
        total_vel = add_vectors(total_vel, scale_vector(wind_scale, wind_vel));
    }
    ...
    drag_coeff = pow(10.0, lin_interp(air_log_re, air_log_drag_coeff, log10(re), table_size));
    df_mag = 0.5 * drag_coeff * AIR_DENSITY * (wind_speed * wind_speed) * (M_PI * (TUX_DIAMETER * TUX_DIAMETER) * 0.25);
    check_assertion(df_mag > 0, "Negative wind force");
    last_time_called = g_game.time;
    return scale_vector(df_mag, total_vel);
}

static vector_t calc_spring_force(scalar_t compression, vector_t vel, vector_t surf_nml, vector_t *unclamped_f)
{
    scalar_t spring_vel; /* velocity perp. to surface (for damping) */
    scalar_t spring_f_mag; /* magnitude of force */

    check_assertion(compression >= 0, "spring can't have negative compression");
    spring_vel = dot_product(vel, surf_nml);
    /* Stage 1 */
    spring_f_mag  = min(compression, TUX_GLUTE_STAGE_1_COMPRESSIBILITY) * TUX_GLUTE_STAGE_1_SPRING_COEFF;
    /* Stage 2 */
    spring_f_mag += max(0, min(compression - TUX_GLUTE_STAGE_1_COMPRESSIBILITY, TUX_GLUTE_STAGE_2_COMPRESSIBILITY)) * TUX_GLUTE_STAGE_2_SPRING_COEFF;
    /* Stage 3 */
    spring_f_mag += max(0., compression - TUX_GLUTE_STAGE_2_COMPRESSIBILITY - TUX_GLUTE_STAGE_1_COMPRESSIBILITY) * TUX_GLUTE_STAGE_3_SPRING_COEFF;
    /* Damping */
    spring_f_mag -= spring_vel * (compression <= TUX_GLUTE_STAGE_1_COMPRESSIBILITY ? TUX_GLUTE_STAGE_1_SPRING_COEFF : 
...
static vector_t calc_net_force(player_data_t *plyr, point_t pos, vector_t vel)
{
    ...
    surf_nml = adjust_surf_nml_for_roll(plyr, vel, surf_fric_coeff, orig_surf_nml);
    ...
    if (dist_from_surface <= -comp_depth) {
        /* Tux ended up below the surface. Calculate the spring force exterted by his rear end. :-) */
        glute_compression = -dist_from_surface - comp_depth;
        check_assertion(glute_compression >= 0, "unexpected negative compression");
        nml_f = calc_spring_force(glute_compression, vel, surf_nml, &unclamped_nml_f);
    }
    ...
    if ((plyr->control.jumping) && (g_game.time - plyr->control.jump_start_time < JUMP_FORCE_DURATION)) {
        jump_f = make_vector(0, BASE_JUMP_G_FORCE * TUX_MASS * EARTH_GRAV + plyr->control.jump_amt * (MAX_JUMP_G_FORCE-BASE_JUMP_G_FORCE) * TUX_MASS * EARTH_GRAV, 0);
    } else {
    ...
    if (dist_from_surface < 0 && speed > MIN_FRICTION_SPEED) {
        ...
        if (fabs(fric_f_mag * sin(ANGLES_TO_RADIANS(steer_angle))) > MAX_TURN_PERPENDICULAR_FORCE) {
            check_assertion(fabs(plyr->control.turn_fact) > 0, "steer angle is non-zero when player is not turning");
            steer_angle = RADIANS_TO_ANGLES(asin(MAX_TURN_PERPENDICULAR_FORCE / fric_f_mag)) * plyr->control.turn_fact / fabs(plyr->control.turn_fact);
        }
        make_rotation_about_vector_matrix(fric_rot_mat, orig_surf_nml, steer_angle);
        ...
        if (speed > get_min_tux_speed() && plyr->control.is_braking) {
            brake_f = scale_vector(surf_fric_coeff * BRAKE_FORCE, fric_dir); 
        } else {
    ...
    if (plyr->control.is_paddling) {
        if (plyr->airborne) {
            ...
            paddling_f = scale_vector(-1 * min(MAX_PADDLING_FORCE, MAX_PADDLING_FORCE * (MAX_PADDLING_SPEED - speed) / MAX_PADDLING_SPEED * min(1.0, surf_fric_coeff / IDEAL_PADDLING_FRIC_COEFF)), fric_dir);
        }
    } else {
        paddling_f = make_vector(0., 0., 0.);
    }
    /* Add all the forces */
    net_force = add_vectors(jump_f, add_vectors(grav_f, add_vectors(nml_f, add_vectors(fric_f, add_vectors(air_f, add_vectors(brake_f, paddling_f))))));
    return net_force;
}
...
/* Solves the system of ordinary differential equations governing Tux's movement.
   Based on Matlab's ode23.m, (c) The MathWorks, Inc. */
void solve_ode_system(player_data_t *plyr, scalar_t dtime)
{
    ...
    case EULER: solver = new_euler_solver(); break;
    case ODE23: solver = new_ode23_solver(); break;
    case ODE45: solver = new_ode45_solver(); break;
    default:
    ...
    while (!done) {
        if (t >= tfinal) {
            print_warning(CRITICAL_WARNING, "t >= tfinal in solve_ode_system()");
            break;
        }
        ...
        for (;;) {
            ...
            /* We assume that the first estimate in all ODE solvers is equal to the final value of the last iteration */
            ...
            /* If the current solver can provide error estimates, update h based on the error, 
               and re-evaluate this step if the error is too large */
            if (solver.estimate_error != NULL) {
                ...
                print_debug(DEBUG_ODE, "pos_err: %g, vel_err: %g", tot_pos_err, tot_vel_err);
                ...
                if (err > tol && h > MIN_TIME_STEP + EPS) {
                    ...
                    if (!failed) {
                        failed = True;
                        h *=  max(0.5, 0.8 * pow(tol / err, solver.time_step_exponent()));
                    } else {
                        h *= 0.5;
                    }
...
/* Updates Tux's position taking into account gravity, friction, tree collisions, etc. 
   This is the main physics function. */
void update_player_pos(player_data_t *plyr, scalar_t dtime)
{
    ...
    adjust_orientation(plyr, dtime, plyr->pos, plyr->vel, dist_from_surface, surf_nml);
    ...
    local_force = rotate_vector(quaternion_conjugate(plyr->orientation), plyr->net_force);

    if (plyr->control.jumping) {
        flap_factor = (g_game.time - plyr->control.jump_start_time) / JUMP_FORCE_DURATION;
    } 
    adjust_tux_joints(plyr->control.turn_animation, plyr->control.is_braking, paddling_factor, speed, local_force, flap_factor);
}
