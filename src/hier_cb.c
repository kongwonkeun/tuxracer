...
int tux_rotate( ClientData cd, Tcl_Interp *ip, int argc, char *argv[] ) 
{
    ...
    if ('x' != axis && 'y' != axis && 'z' != axis) {
        Tcl_AppendResult(ip, argv[0], ": invalid rotation axes", (char *)0 );
        return TCL_ERROR;
    }
    
    /* obtain the angle */
    if (TCL_OK != Tcl_GetDouble(ip, argv[3], &angle)) {
        Tcl_AppendResult(ip, argv[0], ": invalid rotation angle", (char *)0 );
        return TCL_ERROR;
    }
...
int tux_translate( ClientData cd, Tcl_Interp *ip, int argc, char *argv[] ) 
{
    ...
    if (3 != argc) {
        Tcl_AppendResult(ip, argv[0], ": invalid number of arguments\n", "Usage: ", argv[0], " <node> { <x> <y> <z> }", (char *)0 );
        return TCL_ERROR;
    }
    ...
    if (TCL_OK != get_tcl_tuple(ip,argv[2],vec,3)) {
        Tcl_AppendResult(ip, argv[0], ": invalid translation vector", (char *)0 );
        return TCL_ERROR;
    }
...
int tux_scale( ClientData cd, Tcl_Interp *ip, int argc, char *argv[] ) 
{
    ...
    if (4 != argc) {
        Tcl_AppendResult(ip, argv[0], ": invalid number of arguments\n", "Usage: ", argv[0], " <node> { <origin> } { <translation vector> }", (char *)0 );
        return TCL_ERROR;
    }
    /* obtain the nodename */
    nodename = argv[1];
    ...
    if (TCL_OK != get_tcl_tuple(ip,argv[2],origin,3)) {
        Tcl_AppendResult(ip, argv[0], ": invalid origin point", (char *)0 );
        return TCL_ERROR;
    }
    /* obtain the scale factors */
    if (TCL_OK != get_tcl_tuple(ip,argv[3],factors,3)) {
        Tcl_AppendResult(ip, argv[0], ": invalid scale factors", (char *)0 );
        return TCL_ERROR;
    }
...
int tux_transform( ClientData cd, Tcl_Interp *ip, int argc, char *argv[] ) 
{
    ...
    if (3 != argc) {
        Tcl_AppendResult(ip, argv[0], ": invalid number of arguments\n", "Usage: ", argv[0], " <parent node> <child node>", (char *)0 );
        return TCL_ERROR;
    }
...
int tux_sphere( ClientData cd, Tcl_Interp *ip, int argc, char *argv[] ) 
{
    ...
    if (4 != argc) {
        Tcl_AppendResult( ip, argv[0], ": wrong number of arguments\n", "Usage: ", argv[0], " <parent node> <child node> <radius>", (char*) 0 );
        return TCL_ERROR;
    }
    ...
    if ( TCL_OK != Tcl_GetDouble( ip, argv[3], &resolution ) ) {
        Tcl_AppendResult( ip, argv[0], ": resolution is invalid", (char*) 0 );
        return TCL_ERROR;
    }
...
int tux_material( ClientData cd, Tcl_Interp *ip, int argc, char *argv[] ) 
{
    ...
    if (5 != argc) {
        Tcl_AppendResult(ip, argv[0], ": invalid number of arguments\n", "Usage: ", argv[0], " <name> { <ambient colour> } { <specular colour> } <specular exponent", (char *)0 );
        return TCL_ERROR;
    }
    ...
    if (TCL_OK != get_tcl_tuple(ip,argv[2],diffuse,3)) {
        Tcl_AppendResult(ip, argv[0], ": invalid diffuse colour", (char *)0 );
        return TCL_ERROR;
    }

    /* obtain specular colour */
    if (TCL_OK != get_tcl_tuple(ip,argv[3],specular,3)) {
        Tcl_AppendResult(ip, argv[0], ": invalid specular colour", (char *)0 );
        return TCL_ERROR;
    }

    /* obtain specular exponent */
    if (TCL_OK != Tcl_GetDouble(ip,argv[4],&spec_exp)) {
        Tcl_AppendResult(ip, argv[0], ": invalid specular exponent", (char *)0 );
        return TCL_ERROR;
    }

    errmsg = create_material(mat_name,make_colour_from_array(diffuse), make_colour_from_array(specular), spec_exp);
...
int tux_surfaceproperty( ClientData cd, Tcl_Interp *ip, int argc, char *argv[] ) 
{
    char *errmsg;
    ...
    if (3 != argc) {
        Tcl_AppendResult(ip, argv[0], ": invalid number of arguments\n", "Usage: ", argv[0], " <node> <material name>", (char *)0 );
        return TCL_ERROR;
    }
...
int tux_shadow( ClientData cd, Tcl_Interp *ip, int argc, char *argv[] ) 
{
    ...
    if (3 != argc) {
        Tcl_AppendResult(ip, argv[0], ": invalid number of arguments\n", "Usage: ", argv[0], " <node> [on|off]", (char *)0 );
        return TCL_ERROR;
    }
...
int tux_eye( ClientData cd, Tcl_Interp *ip, int argc, char *argv[] ) 
{
    ...
    if (3 != argc) {
        Tcl_AppendResult(ip, argv[0], ": invalid number of arguments\n", "Usage: ", argv[0], " <node> [left|right]", (char *)0 );
        return TCL_ERROR;
    }
