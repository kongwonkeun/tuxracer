...
void update_ui_snow(scalar_t time_step, bool_t windy)
{
    ...
    for (i = 0; i < num_particles; i++) {
        ...
        dist_from_push = (pow((pt->x - push_position.x), 2) + pow((pt->y - push_position.y), 2));
        ...
        if (push_timestep > 0) {
            ...
            f.y *= 1.0 / (PUSH_DIST_DECAY * dist_from_push + 1) * size / PARTICLE_SIZE_RANGE;
        }

        /* update velocity */
        v->x += (f.x + (windy ? WIND_FORCE : 0.0) - v->x * AIR_DRAG) * time_step;
    ...
    /* kill off and regenerate particles */
    for (i = 0; i < num_particles; i++)
    {
        particle_t *p = &particles[i];

        if (p->pt.y < -0.05) {
            /* if we have an excess of particles, kill off with 50% probability */
...
void draw_ui_snow(void)
{
    ...
    if (!get_texture_binding(binding, &texture_id)) {
        print_warning(IMPORTANT_WARNING, "couldn't get texture for binding %s", binding);
        texture_id = 0;
    }
    glTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE);
    glBindTexture(GL_TEXTURE_2D, texture_id);
    glColor4f(particle_colour[0], particle_colour[1], particle_colour[2], particle_colour[3]);
    ...
    for (i = 0; i < num_particles; i++) {
        ...
        glBegin(GL_QUADS);
        glTexCoord2f(tex_min->x, tex_min->y); glVertex2f(0, 0);
        glTexCoord2f(tex_max->x, tex_min->y); glVertex2f(size, 0);
        glTexCoord2f(tex_max->x, tex_max->y); glVertex2f(size, size);
        glTexCoord2f(tex_min->x, tex_max->y); glVertex2f(0, size);
        glEnd();
...
void reset_ui_snow_cursor_pos(point2d_t pos) 
{
    ...
    xres = getparam_x_resolution();
    yres = getparam_y_resolution();
    push_position = make_point2d(pos.x / (scalar_t)xres, pos.y / (scalar_t)yres);
    last_push_position = push_position;
...
void push_ui_snow(point2d_t pos)
{
    ...
    xres = getparam_x_resolution();
    yres = getparam_y_resolution();
    push_position = make_point2d(pos.x / xres, pos.y / yres);
    if (!push_position_initialized) {
        last_push_position = push_position;
    }
    push_position_initialized = True;
